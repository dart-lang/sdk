...
testForEach() async {
  ...
  void checkForEach(
      ...
      {ClassElement stop,
       List<ClassElement> skipChildren: const <ClassElement>[],
    ...
    ForEach visit(ClassElement cls) {
      ...
      if (cls == stop) {
        return ForEach.STOP;
      } else if (skipChildren.contains(cls)) {
        return ForEach.SKIP_CHILDREN;
      }
      return ForEach.CONTINUE;
    }

    if (forEachSubtype) {
      classSet.forEachSubtype(visit, ClassHierarchyNode.ALL);
    } else {
      classSet.forEachSubclass(visit, ClassHierarchyNode.ALL);
    }

    Expect.listEquals(expected, visited,
        "Unexpected classes on $cls."
        "forEach${forEachSubtype ? 'Subtype' : 'Subclass'} "
        "(stop:$stop, skipChildren:$skipChildren):\n"
        "Actual: $visited, expected: $expected\n$classSet");
  }

  checkForEach(A, [A, B, D, C, G, F, I, H, E]);
  checkForEach(A, [A], stop: A);
  checkForEach(A, [A, B, C, G, F, I, H, E], skipChildren: [B]);
  checkForEach(A, [A, B, C], skipChildren: [B, C]);
  checkForEach(A, [A, B, C, G], stop: G, skipChildren: [B]);

  checkForEach(B, [B, D, F, I, H, G], forEachSubtype: true);
  checkForEach(B, [B, D], stop: D, forEachSubtype: true);
  checkForEach(B, [B, D, F, G], skipChildren: [F], forEachSubtype: true);
  checkForEach(B, [B], skipChildren: [B], forEachSubtype: true);
  checkForEach(B, [B, D, F, I, H, G], skipChildren: [D], forEachSubtype: true);
