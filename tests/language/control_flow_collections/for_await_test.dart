...
Future<void> testList() async {
  ...
  // Await in condition.
  Expect.listEquals(list, [for (var i = 1; await Future.value(i < 4); i++) i]);

  // Await in increment.
  Expect.listEquals(list, [for (var i = 1; i < 4; await Future(() => i++)) i]);

  // Await in for body.
  Expect.listEquals(list, [for (var i = 1; i < 4; i++) await Future.value(i)]);
}
...
Future<void> testMap() async {
  ...
  // Await in condition.
  Expect.mapEquals(map, {for (var i = 1; await Future.value(i < 4); i++) i: i});

  // Await in increment.
  Expect.mapEquals(map, {for (var i = 1; i < 4; await Future(() => i++)) i: i});

  // Await in for body key.
  Expect.mapEquals(map, {for (var i = 1; i < 4; i++) await Future.value(i): i});

  // Await in for body value.
  Expect.mapEquals(map, {for (var i = 1; i < 4; i++) i: await Future.value(i)});
}
...
Future<void> testSet() async {
  ...
  // Await in condition.
  Expect.setEquals(set, {for (var i = 1; await Future.value(i < 4); i++) i});

  // Await in increment.
  Expect.setEquals(set, {for (var i = 1; i < 4; await Future(() => i++)) i});

  // Await in for body.
  Expect.setEquals(set, {for (var i = 1; i < 4; i++) await Future.value(i)});
}
