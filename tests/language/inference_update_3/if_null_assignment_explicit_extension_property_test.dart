...
main() {
  ...
  //     (Testing this case here. Otherwise continued below.)
  {
    ...
    contextIterable(
      (Extension('').pIterableIntQuestion ??= iterableDouble)
    ...
    context<Function>(
      (Extension('').pFunctionQuestion ??= callableClassInt)
  ...
  //   - Otherwise, if NonNull(T1) <: S and T2' <: S, then the type of `e` is S.
  {
    ...
    contextB1(
      (Extension('').pC1IntQuestion ??= c2Double)
    ...
    context<Iterable<num>>(
      (Extension('').pIterableIntQuestion ??= listNum)
        ..expectStaticType<Exactly<Iterable<num>>>(),
    );
    ...
    context<B1<int> Function()>(
      (Extension('').pC1IntFunctionQuestion ??= callableClassC2Int)
  ...
  //   - Otherwise, the type of `e` is T.
  {
    ...
    if (o is int?) {
      ...
      // We avoid having a compile-time error because `o` can be demoted.
      o =
          (Extension('').pDoubleQuestion ??= intQuestion)
