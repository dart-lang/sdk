...
class Test extends Base {
  test() {
    ...
    //       `operator[]=` imposes on its second argument).
    {
      ...
      super.pString ??= contextType('')..expectStaticType<Exactly<Object?>>();

      super.pStringQuestion ??= contextType('')
    ...
    //     (Testing this case here. Otherwise continued below.)
    {
      ...
      contextIterable(
        (super.pIterableIntQuestion ??= iterableDouble)
      ...
      context<Function>(
        (super.pFunctionQuestion ??= callableClassInt)
    ...
    //     S.
    {
      ...
      context<Iterable<num>>(
        (super.pIterableIntQuestion ??= listNum)
          ..expectStaticType<Exactly<Iterable<num>>>(),
      );
