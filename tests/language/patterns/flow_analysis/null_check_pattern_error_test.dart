...
test() {
  {
    // A null-check pattern may fail to match.
    int? reachability0 = 0;
  ...
  {
    // If the null-check is applied to a subpattern, the scrutinee is not
    // promoted.
    var x = expr<Object?>();
  ...
  {
    // If the matched value type is `Null`, then a null-check pattern is
  ...
  {
    // If the scrutinee is already promoted, a subsequent null-check pattern
    // doesn't demote it.
    var x = expr<Object?>();
  ...
  {
    // Inside a record pattern, a null-check pattern can promote the record's
    // field type.
    var x = expr<(int?,)>();
    if (x case (_?,)) {
      x.expectStaticType<Exactly<(int,)>>();
