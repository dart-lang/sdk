  ...
  for (int i = 0; i < list.length; i += increment) {
    for (int j = i; j < list.length; j += increment) {
      // Normal
      {
        // Using add/close
        var results;
        var sink = ChunkedConversionSink<String>.withCallback((v) {
          results = v;
        });
      ...
      for (int j = i; j < encoded.length; j += increment) {
        {
          ...
          var sink = ChunkedConversionSink<List<int>>.withCallback((v) {
            results = v;
          });
...
void testErrors() {
  ...
  for (int i = 0; i < 128; i++) {
    if (!units.contains(i)) {
      badDecode(String.fromCharCode(i) * 4);
    }
  }
  ...
  badEncode(0x1000);
  badEncode(0x10000);
  badEncode(0x80000000);
  // The input validation uses bitwise operators, and JS bitwise operators
  // truncate to 32 bits.
  if (jsNumbers) return;
