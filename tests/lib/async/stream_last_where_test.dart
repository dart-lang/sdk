...
void main() {
  if (checkedParameters) {
    ...
    A aFunc() => const A();
    // `lastWhere` does not dynamically allow you to return
    // instances that are not subtypes of the stream element type.
    Expect.throws<TypeError>(() {
      // `aFunc` is not a `B Function()`.
      // Covariance by generics should be checked at call-time.
      stream.lastWhere((x) => false, orElse: aFunc);
    });
  }
  ...
  // Returning a subtype is fine.
  {
    ...
    // `lastWhere` does allow you to return instances of subtypes the
    // stream element type.
