...
expectedFunction() {
  ...
  print(p[0]);  // MSAN: use-of-uninitialized-value
  free(p);
}
...
main(List<String> arguments) {
  ...
  Expect.contains("MemorySanitizer: use-of-uninitialized-value", result.stderr);
  Expect.contains(", 1)", result.stderr);  //# int8: ok
  Expect.contains(", 1)", result.stderr);  //# uint8: ok
  Expect.contains(", 2)", result.stderr);  //# int16: ok
  Expect.contains(", 2)", result.stderr);  //# uint16: ok
  Expect.contains(", 4)", result.stderr);  //# int32: ok
  Expect.contains(", 4)", result.stderr);  //# uint32: ok
  Expect.contains(", 8)", result.stderr);  //# int64: ok
