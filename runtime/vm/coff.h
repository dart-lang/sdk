...
class CoffWriter : public SharedObjectWriter {
 public:
  ...
  // -10..-12 sentinels used for global section-symbol references.
 ...
 private:
  ...
  struct Section : public ZoneObject {
    ...
    // Raw bytes for the section (empty for .bss).
    ZoneGrowableArray<uint8_t>* data = nullptr;
  ...
  struct CoffSym {
    ...
    int32_t value = 0;
    int16_t section_number = 0;  // 1-based, 0 = undef, -1 = abs
    uint16_t type = 0;
