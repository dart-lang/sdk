...
RawArray* ArgumentsDescriptor::New(intptr_t type_args_len,
  ...
  descriptor.SetAt(
      kTypeArgsLenIndex,
      Smi::Handle(Smi::New(TypeArgsChecksField::encode(type_arg_bits) |
  ...
  for (intptr_t i = 0; i < num_named_args; i++) {
    ...
    pos = Smi::New(NamedCheckField::encode((named_arg_bits >> i) & 1) |
...
RawArray* ArgumentsDescriptor::NewNonCached(intptr_t type_args_len,
  ...
  descriptor.SetAt(
      kTypeArgsLenIndex,
      Smi::Handle(zone, Smi::New(TypeArgsLenField::encode(type_args_len) |
