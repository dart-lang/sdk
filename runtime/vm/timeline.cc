...
void Timeline::StreamTo(const char* recorder_kind,
                        const char* file,
                        const char* streams) {
  Timeline::Cleanup();
  ...
  if (strcmp(recorder_kind, "perfettofile") == 0) {
    recorder =
        new TimelineEventPerfettoFileRecorder(file, /*intern_strings=*/true);
  ...
  // Caller must have checked all preconditions.
  RELEASE_ASSERT(recorder != nullptr);
