...
DEFINE_RUNTIME_ENTRY(TypeCheck, 7) {
  ...
  if (!is_instance_of) {
    ...
    if (dst_name.raw() ==
      ...
      const intptr_t receiver_index = args_desc.TypeArgsLen() > 0 ? 1 : 0;
      const intptr_t arg_count = receiver_index + args_desc.Count();
      ...
      const auto& receiver =
          Closure::CheckedHandle(zone, orig_arguments.At(receiver_index));
