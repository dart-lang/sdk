...
class Timeline : public AllStatic {
 public:
  ...
  // Start streaming timeline using given |recorder| (perfettofile, file or
  // systrace).
  //
  // Returns true on success, otherwise returns false and stores error message
  // into |error|.
  static bool StreamTo(const char* recorder,
                       const char* file,
                       const char* streams,
                       const char** error);
