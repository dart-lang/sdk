...
void RawTypedData::WriteTo(SnapshotWriter* writer,
  ...
  if ((kind == Snapshot::kMessage) &&
    ...
    writer->WriteRawPointerValue(passed_data);  // data
    writer->WriteRawPointerValue(passed_data);  // peer
    writer->WriteRawPointerValue(
        reinterpret_cast<void*>(IsolateMessageTypedDataFinalizer));
  } else {
    // Write as internal.
