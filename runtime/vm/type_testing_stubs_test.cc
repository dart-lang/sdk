...
ISOLATE_UNIT_TEST_CASE(TTS_Future) {
  ...
  const auto& type_string = Type::Handle(
      IsolateGroup::Current()->object_store()->non_nullable_string_type());
  const auto& type_num = Type::Handle(
      IsolateGroup::Current()->object_store()->non_nullable_number_type());
  ...
  const auto& type_function = Type::Handle(
      IsolateGroup::Current()->object_store()->non_nullable_function_type());
  const auto& type_legacy_function = Type::Handle(
      IsolateGroup::Current()->object_store()->legacy_function_type());
  // ->function_type() is legacy in non-NNBD mode, so force nullability.
