...
static void ReleasePathBuffer(void* isolate_callback_data,
                              Dart_WeakPersistentHandle handle,
                              void* peer) {
  free(peer);
}
...
void FUNCTION_NAME(Path_GetRawPath)(Dart_NativeArguments args) {
  ...
  char* path_buffer = strdup(path->raw_path());
  Dart_Handle list = Dart_NewExternalTypedDataWithFinalizer(
      Dart_TypedData_kUint8, path_buffer, path->length(), path_buffer,
      path->length(), ReleasePathBuffer);
