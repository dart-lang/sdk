...
SocketAddress::SocketAddress(struct addrinfo* addrinfo) {
  ...
  sockaddr_in *sockaddr = SocketAddress::GetAsSockAddrIn(addrinfo->ai_addr);
...
intptr_t Socket::CreateConnect(sockaddr_storage addr, const intptr_t port) {
  ...
  intptr_t result = TEMP_FAILURE_RETRY(
      connect(fd,
              SocketAddress::GetAsSockAddr(&addr),
...
intptr_t Socket::GetPort(intptr_t fd) {
  ...
  if (TEMP_FAILURE_RETRY(
          getsockname(fd,
                      SocketAddress::GetAsSockAddr(&socket_address),
                      &size))) {
    Log::PrintErr("Error getsockname: %s\n", strerror(errno));
    return 0;
  }
  return SocketAddress::GetAddrPort(&socket_address);
}
...
bool Socket::GetRemotePeer(intptr_t fd, char *host, intptr_t *port) {
  ...
  struct sockaddr* as_addr = SocketAddress::GetAsSockAddr(&socket_address);
  const void* src;
  if (socket_address.ss_family == AF_INET6) {
    src = reinterpret_cast<const void*>(
        &SocketAddress::GetAsSockAddrIn6(as_addr)->sin6_addr);
  } else {
    src = reinterpret_cast<const void*>(
        &SocketAddress::GetAsSockAddrIn(as_addr)->sin_addr);
  }
  ...
  *port = SocketAddress::GetAddrPort(&socket_address);
  return true;
}
