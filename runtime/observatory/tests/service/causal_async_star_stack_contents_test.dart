...
//
// dart pkg/vm_service/test/update_line_numbers.dart runtime/observatory/tests/service/causal_async_star_stack_contents_test.dart
//
const LINE_0 = 33;
const LINE_B = 34;
const LINE_1 = 35;
...
helper() async {
  ...
  debugger(); // LINE_2
  print('helper'); // LINE_A
  await for (var i in foobar()) { // LINE_3
    print('helper $i');
  }
}
...
var tests = <IsolateTest>[
  ...
  (Isolate isolate) async {
    ...
    expect(asyncStack[1].kind, equals(M.FrameKind.asyncSuspensionMarker));
    expect(asyncStack[2].toString(), contains('helper'));
    expect(await asyncStack[2].location.toUserString(), contains('.dart:$LINE_3'));
    expect(asyncStack[3].kind, equals(M.FrameKind.asyncSuspensionMarker));
  },
];
