...
class Isolate extends ServiceObjectOwner implements M.Isolate {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    libraries.clear();
    libraries.addAll(List.castTo<dynamic, Library>(map['libraries']));
    ...
    if (map['extensionRPCs'] != null) {
      extensionRPCs.addAll(List.castTo<dynamic, String>(map['extensionRPCs']));
    }

    threads.clear();
    if (map['_threads'] != null) {
      threads.addAll(List.castTo<dynamic, Thread>(map['_threads']));
    }
...
class Library extends HeapObject implements M.Library {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    classes.clear();
    classes.addAll(List.castTo<dynamic, Class>(map['classes']));
    classes.sort(ServiceObject.LexicalSortName);
    variables.clear();
    variables.addAll(List.castTo<dynamic, Field>(map['variables']));
    variables.sort(ServiceObject.LexicalSortName);
    functions.clear();
    functions.addAll(List.castTo<dynamic, ServiceFunction>(map['functions']));
...
class Class extends HeapObject implements M.Class {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    subclasses.clear();
    subclasses.addAll(List.castTo<dynamic, Class>(map['subclasses']));
    subclasses.sort(ServiceObject.LexicalSortName);

    interfaces.clear();
    interfaces.addAll(List.castTo<dynamic, Instance>(map['interfaces']));
    interfaces.sort(ServiceObject.LexicalSortName);

    fields.clear();
    fields.addAll(List.castTo<dynamic, Field>(map['fields']));
