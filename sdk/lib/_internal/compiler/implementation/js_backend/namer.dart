...
class Namer implements ClosureNamer {
  ...
  String instanceMethodName(FunctionElement element) {
    ...
    // We don't mangle the closure invoking function name if no named arguments
    // are present because it is generated by string concatenation in
    // applyFunction from js_helper.dart. To keep code size down, we potentially
    // shorten the prefix though. Also, in the presence of named arguments, we
    // can use the usual minification path.
    String methodName;
  ...
  String invocationName(Selector selector) {
    if (selector.isGetter()) {
      ...
      // We don't mangle the closure invoking function name if no named
      // arguments are present because it is generated by string concatenation
      // in applyFunction from js_helper.dart. To keep code size down, we
      // potentially shorten the prefix though. Also, in the presence of named
      // arguments, we can use the usual minification path.
      String proposedName;
      if (selector.isClosureCall()) {
        proposedName = callPrefix;
        if (selector.namedArguments.isEmpty) return "$proposedName$suffix";
      } else {
         proposedName = privateName(selector.library, name);
      }
