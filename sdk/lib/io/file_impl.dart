...
/// called.
///
/// The adapter coalesces small [add]s into a single write if there is already
/// a write pending. This prevents the pathological case where
/// [RandomAccessFile.writeFrom] is called once per byte [add]ed.
///
/// ```dart import:async import:core
/// final file = File('<some path>');
/// final sink = file.openWrite();
/// sink.add(<int>[1]); // Immediately written using `writeFrom(<int>[1])`.
/// sink.add(<int>[2]); // Cannot be written, `writeFrom` still executing.
/// sink.add(<int>[3]); // ...
/// sink.add(<int>[4]); // ...
/// sink.add(<int>[5]); // ...
/// // When `RandomAccessfile.writeFrom(<int>[1])` completes and the event
/// // loop runs (or a call to `add` or `flush` is made), all of the small
/// // `add`s are accumulated into a single buffer and written together i.e.
/// // `RandomAccessfile.writeFrom(<int>[2, 3, 4, 5])`.
