...
class EnumSetTest {
  void test_60bits() {
    expect(_MyEnum.values.length, 60);

    for (var constant in _MyEnum.values) {
      var enumSet = EnumSet<_MyEnum>.empty();
      expect(enumSet[constant], false);

      // Can set.
      enumSet = enumSet.updated(constant, true);
      expect(enumSet[constant], true, reason: '$constant');

      // Can clear.
      enumSet = enumSet.updated(constant, false);
      expect(enumSet[constant], false);
    }
  }
