...
class VsCodeExtensionsManager {
  ...
  Future<VsCodeExtensionResults> findVsCodeExtensions(String rootPath) async {
    ...
    final packageConfig = Uri.file(
      path.join(rootPath, '.dart_tool', 'package_config.json'),
    );
    ...
    try {
      ...
      for (final extension in extensions) {
        try {
          ...
          results.parseErrors.add(
            VsCodeExtensionParseError(
...
class VsCodeExtensionParseError {
  ...
  Map<String, Object?> toJson() => {'packageName': packageName, 'error': error};
}
...
class VsCodeExtensionConfig {
  ...
  factory VsCodeExtensionConfig.parse(
    ...
    if (json case {vsCodeExtensionIdKey: final String vsCodeExtensionId}) {
      return VsCodeExtensionConfig._(
        packageName: packageName,
        vsCodeExtensionId: vsCodeExtensionId,
      );
    } else {
      const requiredKeysFromConfigFile = <String>{vsCodeExtensionIdKey};
