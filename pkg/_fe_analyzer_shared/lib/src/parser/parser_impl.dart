...
class Parser {
      ...
      if (next.isA(TokenType.EQ)) {
        ...
        if (!type.isFunctionType) {
          ...
          if (skippedType.next!.isA(TokenType.OPEN_PAREN) &&
            ...
            reportRecoverableError(
              functionToken,
              codes.codeExpectedButGot.withArguments('Function'),
            );
  ...
  Token parseMixinApplicationRest(Token token) {
    ...
    if (!withKeyword.isA(Keyword.WITH)) {
      ...
      reportRecoverableError(
        withKeyword,
        codes.codeExpectedButGot.withArguments('with'),
      );
  ...
  Token parseEnumHeaderOpt(Token token, Token enumKeyword) {
    ...
    while (!_isOneOfFollowingValues(token.next!)) {
      ...
      Token? skipToken =
          recoveryEnumWith(
            token,
            codes.codeMultipleClauses.withArguments("enum", "with"),
          ) ??
    ...
    while (!token.next!.isA(TokenType.OPEN_CURLY_BRACKET)) {
      ...
      Token? skipToken = recoveryEnumWith(
        ...
        hasWithClauses
            ? codes.codeMultipleClauses.withArguments("enum", "with")
            : codes.codeOutOfOrderClauses.withArguments("with", "implements"),
      );
      if (skipToken != null) {
        hasWithClauses = true;
      }
      if (skipToken == null) {
        ...
        skipToken = recoveryEnumImplements(
          token,
          codes.codeMultipleClauses.withArguments("enum", "implements"),
        );
      }
      if (skipToken == null) {
  ...
  Token? recoverySmallLookAheadSkipTokens(
    ...
    if (foundWanted) {
      ...
      if (skipped == 1) {
        reportRecoverableError(
          skipToken,
          codes.codeUnexpectedToken.withArguments(skipToken),
        );
      } else {
  ...
  Token parseDeclarationHeaderRecoveryInternal(
    ...
    do {
      ...
      if (token.next!.isKeywordOrIdentifier &&
        ...
        reportRecoverableError(
          token.next!,
          codes.codeExpectedInstead.withArguments('extends'),
        );
  ...
  Token parseMixinHeaderRecovery(
    ...
    do {
      ...
      if (token.next!.isKeywordOrIdentifier &&
        ...
        reportRecoverableError(
          token.next!,
          codes.codeExpectedInstead.withArguments('on'),
        );
    ...
    if (augmentToken != null) {
      ...
      if (!onKeyword.isA(Keyword.ON)) {
        ...
        if (onKeyword.isA(Keyword.EXTENDS) ||
          ...
          reportRecoverableError(
            token,
            codes.codeExpectedAfterButGot.withArguments('on'),
          );
    ...
    if (next.isA(TokenType.EQ)) {
      ...
      if (varFinalOrConst != null && !name.isSynthetic) {
        if (varFinalOrConst.isA(Keyword.CONST)) {
          reportRecoverableError(
            name,
            codes.codeConstFieldWithoutInitializer.withArguments(name.lexeme),
          );
          ...
          reportRecoverableError(
            name,
            codes.codeFinalFieldWithoutInitializer.withArguments(name.lexeme),
          );
        }
      }
  ...
  Token ensureCloseParen(Token token, Token openParen) {
    ...
    reportRecoverableError(next, codes.codeExpectedButGot.withArguments(')'));
  ...
  Token ensureColon(Token token) {
    ...
    codes.Message message = codes.codeExpectedButGot.withArguments(':');
  ...
  Token ensureLiteralString(Token token) {
    Token next = token.next!;
    if (next.kind != STRING_TOKEN) {
      codes.Message message = codes.codeExpectedString.withArguments(next);
  ...
  Token parseFunctionBody(
    ...
    while (notEofOrType(TokenType.CLOSE_CURLY_BRACKET, token.next!)) {
      ...
      if (identical(token.next!, startToken)) {
        ...
        reportRecoverableError(
          token,
          codes.codeUnexpectedToken.withArguments(token),
        );
        token = token.next!;
      }
      ++statementCount;
  ...
  Token _parsePrecedenceExpressionLoop(
    ...
    if (constantPatternContext != ConstantPatternContext.none &&
      ...
      if (constantPatternContext == ConstantPatternContext.explicit) {
        ...
        reportRecoverableError(
          next,
          codes.codeInvalidConstantPatternBinary.withArguments(type.lexeme),
        );
      } else {
  ...
  bool _attemptPrecedenceLevelRecovery(
    ...
    for (int i = 0; i < replacements.length; i++) {
      ...
      if (acceptRecovery) {
        ...
        reportRecoverableError(
          token.next!,
          codes.codeBinaryOperatorWrittenOut.withArguments(
            token.next!.lexeme,
            replacement.lexeme,
          ),
        );
  ...
  Token parseUnaryExpression(
    ...
    if (identical(value, 'await')) {
      ...
      if (constantPatternContext != ConstantPatternContext.none) {
        reportRecoverableError(
          operator,
          codes.codeInvalidConstantPatternUnary.withArguments(value!),
        );
      }
      // Right associative, so we recurse at the same precedence
      // level.
  ...
  Token ensureParenthesizedCondition(Token token, {required bool allowCase}) {
    ...
    if (!openParen.isA(TokenType.OPEN_PAREN)) {
      ...
      reportRecoverableError(
        openParen,
        codes.codeExpectedToken.withArguments('('),
      );
      ...
      if ((value == "Map" || value == "Set") &&
        ...
        if (afterToken.isA(TokenType.OPEN_CURLY_BRACKET)) {
          ...
          reportRecoverableErrorWithEnd(
            newKeyword,
            identifier,
            codes.codeLiteralWithClassAndNew.withArguments(
              value.toLowerCase(),
              identifier,
            ),
          );
    ...
    if ((lexeme == "Map" || lexeme == "Set") &&
      ...
      if (afterToken.isA(TokenType.OPEN_CURLY_BRACKET)) {
        ...
        if (identical(nextValue, '{')) {
          ...
          reportRecoverableError(
            next,
            codes.codeLiteralWithClass.withArguments(
              lexeme.toLowerCase(),
              next,
            ),
          );
  ...
  Token parseExpressionStatementOrDeclarationAfterModifiers(
    ...
    if (beforeType == start &&
      ...
      if (!looksLikeName(next)) {
        reportRecoverableError(
          next,
          codes.codeExpectedIdentifier.withArguments(next),
        );
    ...
    if (beforeType.next!.isA(TokenType.LT)) {
      ...
      insertSyntheticIdentifier(
        ...
        message: codes.codeExpectedIdentifier.withArguments(beforeType.next!),
      );
      typeInfo = computeType(beforeType, /* required = */ true);
    }
    token = typeInfo.parseType(beforeType, this);
  ...
  Token parseForLoopPartsMid(Token token, Token? awaitToken, Token forToken) {
    ...
    if (next.isA(TokenType.SEMICOLON)) {
      ...
      if (next.isA(TokenType.COLON)) {
        ...
        reportRecoverableError(
          next,
          codes.codeExpectedButGot.withArguments('in'),
        );
        token.setNext(
  ...
  Token parseDoWhileStatement(Token token) {
    ...
    if (!whileToken.isA(Keyword.WHILE)) {
      reportRecoverableError(
        whileToken,
        codes.codeExpectedButGot.withArguments('while'),
      );
    ...
    while (notEofOrType(TokenType.CLOSE_CURLY_BRACKET, token.next!)) {
      ...
      while (true) {
        ...
        if (identical(value, 'default')) {
          ...
          reportRecoverableError(
            peek,
            codes.codeExpectedToken.withArguments("case"),
          );
    ...
    while (token.next!.kind != EOF_TOKEN) {
      ...
      if ((identical(value, 'case')) ||
        ...
        if (identical(next, startToken)) {
          ...
          reportRecoverableError(
            next,
            codes.codeUnexpectedToken.withArguments(next),
          );
          token = next;
        }
        ++statementCount;
    ...
    if (next.isIdentifier) {
      ...
      if (afterToken.isA(TokenType.OPEN_PAREN) && !potentialTypeArg.recovered) {
        ...
        if (!patternContext.isRefutable || name == '_') {
          ...
          reportRecoverableError(
            firstIdentifier,
            codes.codeIllegalPatternIdentifierName.withArguments(
              firstIdentifier,
            ),
          );
        }
  ...
  Token parseVariablePattern(
    ...
    switch (patternContext) {
      ...
      case PatternContext.assignment:
        ...
        if (!isBareIdentifier) {
          reportRecoverableError(
            token,
            codes.codePatternAssignmentDeclaresVariable.withArguments(
    ...
    if (variableName == '_') {
      ...
      if (illegalPatternIdentifiers.contains(variableName)) {
        reportRecoverableError(
          token,
          codes.codeIllegalPatternAssignmentVariableName.withArguments(token),
        );
      }
      ...
      if (illegalPatternIdentifiers.contains(variableName)) {
        reportRecoverableError(
          token,
          codes.codeIllegalPatternVariableName.withArguments(token),
        );
      }
      if (isBareIdentifier) {
        listener.handleNoType(token);
