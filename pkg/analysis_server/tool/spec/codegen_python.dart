...
library codegen.python;

import 'api.dart';

/**
 * Visitor specialized for generating Python code.
 */
class PythonCodegenVisitor extends HierarchicalApiVisitor {
  /**
  ...
  PythonCodegenVisitor(Api api) : super(api);

  /**
   * Convert the given [TypeDecl] to a Dart type.
   */
  String pythonType(TypeDecl type) {
    if (type is TypeReference) {
      ...
      return pythonType(referencedType);
    } else if (type is TypeList) {
      return 'List<${pythonType(type.itemType)}>';
    } else if (type is TypeMap) {
      return 'Map<${pythonType(type.keyType)}, ${pythonType(type.valueType)}>';
    } else if (type is TypeUnion) {
      return 'dynamic';
    } else {
      throw new Exception("Can't convert to a Python type");
    }
  }
}
