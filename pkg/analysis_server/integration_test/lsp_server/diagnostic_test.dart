...
linter:
  ...
  Future<void> test_plugins() async {
    ...
    // Expect a diagnostic from the plugin. If it never comes, the test
    // will time out and fail. We don't use the analysis progress notifications
    // here because they do not differentiate between server or plugin status
    // and we wouldn't know which one to wait for.
    //
    // TODO(dantup): Update this to use dart/workspace/analysis/complete once
    //  it is implemented.
    await publishedDiagnostics
        .where((params) => params.uri == mainFileUri)
        .expand((params) => params.diagnostics)
        .map((diagnostic) => diagnostic.message.asString)
        .firstWhere(
          (message) => message == 'A diagnostic generated by a plugin.',
        );
  }
