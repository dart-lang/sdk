...
class ContextManagerImpl implements ContextManager {
  ...
  void _scheduleWatchEvent(WatchEvent event) {
    if (!_isInterestingWatchEvent(event)) {
      return;
    }
    callbacks.analysisServer.messageScheduler.add(WatcherMessage(event));
  }

  /// Returns whether [event] can affect analysis-relevant state.
  bool _isInterestingWatchEvent(WatchEvent event) {
    var path = event.path;
    ...
    var isInAnalyzedContext = collection.contexts.any(
      (analysisContext) => analysisContext.contextRoot.isAnalyzed(path),
    );
    return isInAnalyzedContext;
  }
