...
class JsonTest {
  ...
  late var range = Range(start: start, end: end);

  void test_canParse_enumValue_bad() {
    expect(MarkupKind.canParse('NotAMarkupKind', nullLspJsonReporter), isFalse);
  }

  void test_canParse_enumValue_good() {
    expect(MarkupKind.canParse('plaintext', throwingLspJsonReporter), isTrue);
  }

  void test_canParse_enumValue_unknownValueForUnrestrictedEnum_good() {
    expect(SymbolKind.canParse(-1, throwingLspJsonReporter), isTrue);
  }

  void test_canParse_ignoresUnknownFields() {
    expect(
  ...
  void test_canParse_literalStrings_bad() {
  ...
  void test_canParse_literalStrings_good() {
    // The CreateFile type is defined with `{ kind: 'create' }` so the only
    // allowed value for `kind` is "create".
    var canParse = CreateFile.canParse({
      'kind': 'create',
  ...
  void test_canParse_optionalFields() {
    expect(
      RenameFileOptions.canParse(<String, Object>{}, throwingLspJsonReporter),
      isTrue,
    );
    expect(
      RenameFileOptions.canParse({'overwrite': true}, throwingLspJsonReporter),
      isTrue,
    );
    expect(
  ...
  void test_canParse_reports_nestedFieldsOfWrongType() {
  ...
  void test_canParse_reports_nestedNullFields() {
    var reporter = LspJsonReporter('params');
    expect(
      CompletionParams.canParse({
        'position': {'line': 1, 'character': 1},
  ...
  void test_canParse_reports_nestedUndefinedFields() {
    var reporter = LspJsonReporter('params');
    expect(
      CompletionParams.canParse({
        'position': {'line': 1, 'character': 1},
  ...
  void test_canParse_reports_nulls() {
    var reporter = LspJsonReporter('params');
  ...
  void test_canParse_reports_typeNotInAllowedTypes() {
    var reporter = LspJsonReporter('params');
    expect(
  ...
  void test_canParse_reports_undefineds() {
    var reporter = LspJsonReporter('params');
    expect(CreateFile.canParse(<String, dynamic>{}, reporter), isFalse);
    expect(reporter.errors, hasLength(1));
    expect(reporter.errors.first, equals('params.kind must not be undefined'));
  }

  void test_canParse_reports_wrongTypes_nonSpecTypes() {
    var reporter = LspJsonReporter('params');
  ...
  void test_canParse_reports_wrongTypes_specTypes() {
    var reporter = LspJsonReporter('params');
    expect(ClientCapabilities.canParse({'textDocument': 1}, reporter), isFalse);
    expect(
  ...
  void test_canParse_unionsOfLiterals() {
    // Key = value to test
  ...
  void test_fromJson_ignoresUnknownFields() {
    var input = '{"id":1,"invalidField":true,"method":"foo","jsonrpc":"test"}';
  ...
  void test_fromJson_integersInDoubleFields() {
  ...
  void test_fromJson_union_null_notificationMessage() {
  ...
  void test_fromJson_union_null_responseMessage() {
  ...
  void test_fromJson_union_right() {
  ...
  void test_toJson_nestedUnion() {
    var message = ResponseMessage(
      id: Either2<int, String>.t1(1),
      result: Either2<Either2<List<Location>, Location>, List<LocationLink>>.t1(
  ...
  void test_toJson_nestedUnion_map() {
    var message = ResponseMessage(
      id: Either2<int, String>.t1(1),
      result: Either2<Either2<List<Location>, Location>, List<LocationLink>>.t1(
        Either2<List<Location>, Location>.t1([
  ...
  void test_toJson_responseMessage_noError() {
    var id = Either2<int, String>.t1(1);
    var result = 'my result';
    var resp = ResponseMessage(id: id, result: result, jsonrpc: jsonRpcVersion);
    var jsonMap = resp.toJson();
    expect(jsonMap, contains('result'));
    expect(jsonMap, isNot(contains('error')));
  }

  void test_toJson_responseMessage_noResult_error() {
    var id = Either2<int, String>.t1(1);
  ...
  void test_toJson_responseMessage_nullResult_noError() {
    var id = Either2<int, String>.t1(1);
  ...
  void test_toJson_responseMessage_throws_resultAndError() {
    var id = Either2<int, String>.t1(1);
    var result = 'my result';
    var error = ResponseError(code: ErrorCodes.ParseError, message: 'Error');
  ...
  void test_toJson_unionContainingInterfaceType() {
