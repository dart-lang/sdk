...
void main() {
  defineReflectiveSuite(() {
    defineReflectiveTests(FlutterWrapExpandedTest);
  });
}
...
class FlutterWrapExpandedTest extends AssistProcessorTest {
  @override
  AssistKind get kind => DartAssistKind.FLUTTER_WRAP_EXPANDED;
...
''');
  ...
  Future<void> test_aroundExpanded() async {
    await resolveTestCode('''
import 'package:flutter/widgets.dart';
class FakeFlutter {
  Widget f() {
    return /*caret*/Expanded(child: Container());
  }
}
''');
...
Widget f() {
  return Expanded(child: MyWidget.named());
}
''');
  }
...
import 'package:flutter/widgets.dart';
class FakeFlutter {
  Widget f() => Expanded(child: Container());
}
''');
  }
...
class FakeFlutter {
  Widget f() {
    return Column(
      children: [Expanded(child: Text('aaa'))],
    );
  }
}
''');
...
void f() {
  Widget w = Expanded(child: Container());
}
''');
  }
