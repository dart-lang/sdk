...
abstract class PipelineTestStrategy<S extends ModularStep> {
  ...
  FutureOr<Pipeline<S>> createPipeline(
...
void runPipelineTest<S extends ModularStep>(
  ...
  test('can read source data if requested', () async {
    ...
    expect(
      testStrategy.getResult(pipeline, m1, _concatId),
      "a1.dart: A1\na2.dart: A2\n",
    );
  ...
  test('step is applied to all modules', () async {
    ...
    expect(
      testStrategy.getResult(pipeline, m2, _concatId),
      "b/b1.dart: B1\nb/b2.dart: B2\n",
    );
    await testStrategy.cleanup(pipeline);
  });
  ...
  test('can read previous step results if requested', () async {
    ...
    var pipeline = await testStrategy.createPipeline(sources, <S>[
      concatStep,
      lowercaseStep,
    ]);
    await pipeline.run(twoModuleInput);
    expect(
      testStrategy.getResult(pipeline, m1, _lowercaseId),
      "a1.dart: a1\na2.dart: a2\n",
    );
    expect(
      testStrategy.getResult(pipeline, m2, _lowercaseId),
      "b/b1.dart: b1\nb/b2.dart: b2\n",
    );
    await testStrategy.cleanup(pipeline);
  });
  ...
  test('all outputs of a step are created together', () async {
    ...
    expect(
      testStrategy.getResult(pipeline, m2, _uppercaseId),
      "B/B1.DART: B1\nB/B2.DART: B2\n",
    );
    await testStrategy.cleanup(pipeline);
  });
  ...
  test('can read same-step results of dependencies if requested', () async {
    ...
    expect(
      testStrategy.getResult(pipeline, m2, _joinId),
      "a1 a1\na2 a2\n\nb/b1 b1\nb/b2 b2\n",
    );
    await testStrategy.cleanup(pipeline);
  });
  ...
  test(
    'cannot read same-step results of dependencies if not requested',
    () async {
      var concatStep = testStrategy.createSourceOnlyStep(
        action: _concat,
        resultId: _concatId,
  ...
  test(
    'cannot read prior step results of dependencies if not requested',
    () async {
      var concatStep = testStrategy.createSourceOnlyStep(
        action: _concat,
        resultId: _concatId,
  ...
  test('only main applies to main module', () async {
    ...
    expect(
      testStrategy.getResult(pipeline, m3, _joinId),
      "b/b1.dart: b1\nb/b2.dart: b2\n\na1.dart: a1\na2.dart: a2\n\nc c0\n",
    );
    await testStrategy.cleanup(pipeline);
  });
