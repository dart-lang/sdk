...
FLAGS=
if [ "$TRAVIS" ]; then
  # TODO(jmesserly): figure out how to get batch mode working on Travis
  echo Disabling --batch mode on Travis
  FLAGS=--noBatch
fi
...
dart -c tools/testing/dart/main.dart $FLAGS \
    -m release -r chrome -c dartdevc --strong \
