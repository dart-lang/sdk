...
(function() {
  ...
  for (let testFile of dart_library.libraries()) {
    ...
    if (match != null) {
      ...
      if (status.expectedFailures.has(name)) {
      ...
      test(name, (done) => {
        ...
        let mainLibrary = dart_library.import(name)[name];
  ...
  (dart_library.import('async_await_test_none_multi')
      .async_await_test_none_multi.main());
  // TODO(vsm): Re-enable.
  // See https://github.com/dart-lang/dev_compiler/issues/456
  // (dart_library.import('async_star_test_none_multi')
  //     .async_star_test_none_multi.main());
