...
class Latin1Codec extends Encoding {
  ...
  Converter<List<int>, String> get decoder => _allowInvalid
  ...
  void addSlice(List<int> source, int start, int end, bool isLast) {
    ...
    for (int i = start; i < end; i++) {
      ...
      if (char > _LATIN1_MASK || char < 0) {
        ...
        _addSliceToSink(((__x26) => DDC$RT.cast(__x26,
            DDC$RT.type((List<dynamic> _) {}), DDC$RT.type((List<int> _) {}),
            "CastLiteral",
            """line 161, column 25 of dart:convert/latin1.dart: """,
            __x26 is List<int>, false))(const [0xFFFD]), 0, 1, false);
        start = i + 1;
      }
    }
    if (start < end) {
