dart_library.library('corelib/regexp/ascii-regexp-subject_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__ascii$45regexp$45subject_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const ascii$45regexp$45subject_test = Object.create(null);
  ...
  ascii$45regexp$45subject_test.main = function() {
    let s = "foo";
    let i = null;
    for (i = 0; dart.test(dart.dsend(i, '<', 18)); i = dart.dsend(i, '+', 1)) {
      s = s + s;
    }
    function repeatRegexp(re) {
      for (i = 0; dart.test(dart.dsend(i, '<', 1000)); i = dart.dsend(i, '+', 1)) {
        dart.dsend(re, 'hasMatch', s);
      }
    }
    dart.fn(repeatRegexp, dynamicTodynamic());
    repeatRegexp(core.RegExp.new("^bar"));
    repeatRegexp(core.RegExp.new("^foo|^bar|^baz"));
    repeatRegexp(core.RegExp.new("(^bar)"));
    repeatRegexp(core.RegExp.new("(?=^bar)\\w+"));
  };
  dart.fn(ascii$45regexp$45subject_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.ascii$45regexp$45subject_test = ascii$45regexp$45subject_test;
