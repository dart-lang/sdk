dart_library.library('corelib/regexp/compile_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__compile_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const compile_test = Object.create(null);
  ...
  compile_test.main = function() {
    v8_regexp_utils.description('Test RegExp.compile method.');
    let re = core.RegExp.new("a", {caseSensitive: false});
    v8_regexp_utils.shouldBeTrue(re.hasMatch('A'));
    v8_regexp_utils.shouldBeFalse(re.isMultiLine);
    v8_regexp_utils.shouldBeFalse(re.isCaseSensitive);
    re = core.RegExp.new("a");
    v8_regexp_utils.shouldBeFalse(re.isMultiLine);
    v8_regexp_utils.shouldBeTrue(re.isCaseSensitive);
    v8_regexp_utils.shouldBeFalse(re.hasMatch('A'));
    v8_regexp_utils.assertThrows(dart.fn(() => core.RegExp.new('+'), VoidToRegExp()));
  };
  dart.fn(compile_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.compile_test = compile_test;
