dart_library.library('corelib/regexp/invalid-range-in-class_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__invalid$45range$45in$45class_test(exports, dart_sdk, expect) {
  'use strict';
  ...
  const invalid$45range$45in$45class_test = Object.create(null);
  ...
  invalid$45range$45in$45class_test.main = function() {
    v8_regexp_utils.description("This page tests invalid character ranges in character classes.");
    v8_regexp_utils.shouldBe(core.RegExp.new("[a-c]+").firstMatch("-acbd"), JSArrayOfString().of(["acb"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[a\\-c]+").firstMatch("-acbd"), JSArrayOfString().of(["-ac"]));
    v8_regexp_utils.assertThrows(dart.fn(() => core.RegExp.new("[c-a]+").firstMatch("-acbd"), VoidToMatch()));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\d-x]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3x"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[x-\\d]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3x"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\\\d-\\d]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\d-a-z]+").firstMatch("az1-3y"), JSArrayOfString().of(["az1-3"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\d\\-x]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3x"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[x\\-\\d]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3x"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\d\\-\\d]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("[\\d-]+").firstMatch("1-3xy"), JSArrayOfString().of(["1-3"]));
  };
  dart.fn(invalid$45range$45in$45class_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.invalid$45range$45in$45class_test = invalid$45range$45in$45class_test;
