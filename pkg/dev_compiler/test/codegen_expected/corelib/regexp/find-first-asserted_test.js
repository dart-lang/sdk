dart_library.library('corelib/regexp/find-first-asserted_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__find$45first$45asserted_test(exports, dart_sdk, expect) {
  'use strict';
  ...
  const find$45first$45asserted_test = Object.create(null);
  ...
  find$45first$45asserted_test.main = function() {
    v8_regexp_utils.description('Tests some regular expressions that were doing the wrong thing with the "find first asserted" optimization.');
    v8_regexp_utils.shouldBe(core.RegExp.new(".*<body>(.*)</body>.*").firstMatch("foo<body>bar</body>baz"), JSArrayOfString().of(["foo<body>bar</body>baz", "bar"]));
    v8_regexp_utils.shouldBe(core.RegExp.new("\\s*<!--([\\s\\S]*)//\\s*-->\\s*").firstMatch("<!--// -->"), JSArrayOfString().of(["<!--// -->", ""]));
  };
  dart.fn(find$45first$45asserted_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.find$45first$45asserted_test = find$45first$45asserted_test;
