dart_library.library('corelib/regexp/negative-special-characters_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__negative$45special$45characters_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const negative$45special$45characters_test = Object.create(null);
  ...
  negative$45special$45characters_test.main = function() {
    v8_regexp_utils.description("This test checks Unicode in negative RegExp character classes.");
    function testPassed(str) {
    }
    dart.fn(testPassed, dynamicTodynamic());
    function testFailed(str) {
      return expect$.Expect.fail(core.String._check(str));
    }
    dart.fn(testFailed, dynamicTodynamic());
    function test(pattern, str, expected_length) {
      let result = dart.dsend(str, 'replaceAll', core.RegExp.new(core.String._check(pattern), {caseSensitive: false, multiLine: true}), '');
      if (dart.equals(dart.dload(result, 'length'), expected_length))
        testPassed('"' + dart.notNull(core.String._check(pattern)) + '", ' + '"' + dart.notNull(core.String._check(str)) + '".');
      else
        testFailed('"' + dart.notNull(core.String._check(pattern)) + '", ' + '"' + dart.notNull(core.String._check(str)) + '". Was "' + dart.notNull(core.String._check(result)) + '".');
    }
  ...
  dart.fn(negative$45special$45characters_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.negative$45special$45characters_test = negative$45special$45characters_test;
