dart_library.library('corelib/regexp/malformed-escapes_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__malformed$45escapes_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const malformed$45escapes_test = Object.create(null);
  ...
  malformed$45escapes_test.main = function() {
    v8_regexp_utils.description("This page tests handling of malformed escape sequences.");
    let regexp = null;
    regexp = core.RegExp.new("\\ug", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'ug'));
    regexp = core.RegExp.new("\\xg", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'xg'));
    regexp = core.RegExp.new("\\c_", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', '\\\\c_'));
    regexp = core.RegExp.new("[\\B]", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'B'));
    regexp = core.RegExp.new("[\\b]", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', '\b'));
    regexp = core.RegExp.new("\\8", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', '\\8'));
    regexp = core.RegExp.new("^[\\c]$");
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'c'));
    regexp = core.RegExp.new("^[\\c_]$");
    v8_regexp_utils.shouldBeFalse(dart.dsend(regexp, 'hasMatch', 'c'));
    regexp = core.RegExp.new("^[\\c]]$");
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'c]'));
  };
  dart.fn(malformed$45escapes_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.malformed$45escapes_test = malformed$45escapes_test;
