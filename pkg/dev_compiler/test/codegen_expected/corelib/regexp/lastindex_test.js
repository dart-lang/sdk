dart_library.library('corelib/regexp/lastindex_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__lastindex_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const lastindex_test = Object.create(null);
  ...
  lastindex_test.main = function() {
    let re = core.RegExp.new("[^\\s$]+");
    let accumulate = "";
    let match = null;
    for (let match of re.allMatches("  abcdefg")) {
      accumulate = accumulate + (dart.notNull(match.group(0)) + "; ");
    }
    v8_regexp_utils.assertEquals(accumulate, "abcdefg; ");
    re = core.RegExp.new("\\d");
    accumulate = "";
    for (let match of re.allMatches("123456789")) {
      accumulate = accumulate + (dart.notNull(match.group(0)) + "; ");
    }
    v8_regexp_utils.assertEquals(accumulate, "1; 2; 3; 4; 5; 6; 7; 8; 9; ");
  };
  dart.fn(lastindex_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.lastindex_test = lastindex_test;
