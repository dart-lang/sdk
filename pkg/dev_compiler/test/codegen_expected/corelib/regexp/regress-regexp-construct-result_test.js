dart_library.library('corelib/regexp/regress-regexp-construct-result_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__regress$45regexp$45construct$45result_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const regress$45regexp$45construct$45result_test = Object.create(null);
  ...
  regress$45regexp$45construct$45result_test.main = function() {
    let num_captures = 1000;
    let regexp_string = "(a)";
    for (let i = 0; i < num_captures - 1; i++) {
      regexp_string = regexp_string + "|(b)";
    }
    let regexp = core.RegExp.new(regexp_string);
    for (let i = 0; i < 10; i++) {
      let matches = regexp.firstMatch("a");
      let count = 0;
      v8_regexp_utils.assertEquals(num_captures + 1, dart.notNull(matches.groupCount) + 1);
    }
  };
  dart.fn(regress$45regexp$45construct$45result_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.regress$45regexp$45construct$45result_test = regress$45regexp$45construct$45result_test;
