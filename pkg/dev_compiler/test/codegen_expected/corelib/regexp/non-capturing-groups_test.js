dart_library.library('corelib/regexp/non-capturing-groups_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__non$45capturing$45groups_test(exports, dart_sdk, expect) {
  'use strict';
  ...
  const non$45capturing$45groups_test = Object.create(null);
  ...
  non$45capturing$45groups_test.main = function() {
    v8_regexp_utils.description('Test for behavior of non-capturing groups, as described in <a href="http://blog.stevenlevithan.com/archives/npcg-javascript">' + 'a blog post by Steven Levithan</a> and <a href="http://bugs.webkit.org/show_bug.cgi?id=14931">bug 14931</a>.');
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("(x)?\\1y").hasMatch("y"));
    ...
    v8_regexp_utils.shouldBe(v8_regexp_utils.firstMatch("y", core.RegExp.new("(x)?\\1y")), JSArrayOfString().of(["y", null]));
    expect$.Expect.listEquals("y"[dartx.split](core.RegExp.new("(x)?\\1y")), JSArrayOfString().of(["", ""]));
    expect$.Expect.listEquals("y"[dartx.split](core.RegExp.new("(x)?y")), JSArrayOfString().of(["", ""]));
    v8_regexp_utils.assertEquals("y"[dartx.indexOf](core.RegExp.new("(x)?\\1y")), 0);
    v8_regexp_utils.assertEquals("y"[dartx.replaceAll](core.RegExp.new("(x)?\\1y"), "z"), "z");
    v8_regexp_utils.assertEquals("y"[dartx.replaceAllMapped](core.RegExp.new("(x)?y"), dart.fn(m => m.group(1), MatchToString())), "null");
    v8_regexp_utils.assertEquals("y"[dartx.replaceAllMapped](core.RegExp.new("(x)?\\1y"), dart.fn(m => m.group(1), MatchToString())), "null");
    v8_regexp_utils.assertEquals("y"[dartx.replaceAllMapped](core.RegExp.new("(x)?y"), dart.fn(m => m.group(1), MatchToString())), "null");
  };
  dart.fn(non$45capturing$45groups_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.non$45capturing$45groups_test = non$45capturing$45groups_test;
