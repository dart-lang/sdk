dart_library.library('corelib/regexp/quantified-assertions_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__quantified$45assertions_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const quantified$45assertions_test = Object.create(null);
  ...
  quantified$45assertions_test.main = function() {
    v8_regexp_utils.description("This page tests assertions followed by quantifiers.");
    let regexp = null;
    regexp = core.RegExp.new("(?=a){0}", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'a'));
    regexp = core.RegExp.new("(?=a){1}", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'a'));
    regexp = core.RegExp.new("(?!a){0}", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'b'));
    regexp = core.RegExp.new("(?!a){1}", {multiLine: true});
    v8_regexp_utils.shouldBeTrue(dart.dsend(regexp, 'hasMatch', 'b'));
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("^(?=a)?b$").hasMatch("b"));
  };
  dart.fn(quantified$45assertions_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.quantified$45assertions_test = quantified$45assertions_test;
