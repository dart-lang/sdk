dart_library.library('corelib/regexp/backreferences_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__backreferences_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const backreferences_test = Object.create(null);
  ...
  backreferences_test.main = function() {
    v8_regexp_utils.description("Test to ensure correct behaviour when using backreferences in a RegExp");
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("(...)\\1$").hasMatch('abcabc'));
    v8_regexp_utils.shouldBeFalse(core.RegExp.new("(...)\\1$").hasMatch('abcdef'));
    v8_regexp_utils.shouldBeFalse(core.RegExp.new("(...)\\2$").hasMatch('abcabc'));
    v8_regexp_utils.shouldBeFalse(core.RegExp.new("(...)\\2$").hasMatch('abc'));
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("\\1(...)$").hasMatch('abcabc'));
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("\\1(...)$").hasMatch('abcdef'));
    v8_regexp_utils.shouldBeFalse(core.RegExp.new("\\2(...)$").hasMatch('abcabc'));
    v8_regexp_utils.shouldBeFalse(core.RegExp.new("\\2(...)$").hasMatch('abc'));
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("\\1?(...)$").hasMatch('abc'));
    v8_regexp_utils.shouldBeTrue(core.RegExp.new("\\1?(...)$").hasMatch('abc'));
    let re = core.RegExp.new("[^b]*((..)|(\\2))+Sz", {caseSensitive: false});
    v8_regexp_utils.shouldBeFalse(re.hasMatch('axabcd'));
    v8_regexp_utils.shouldBeTrue(re.hasMatch('axabcsz'));
  };
  dart.fn(backreferences_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.backreferences_test = backreferences_test;
