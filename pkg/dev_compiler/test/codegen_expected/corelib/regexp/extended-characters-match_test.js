dart_library.library('corelib/regexp/extended-characters-match_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__extended$45characters$45match_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const extended$45characters$45match_test = Object.create(null);
  ...
  extended$45characters$45match_test.main = function() {
    v8_regexp_utils.description("This test checks regular expressions using extended (> 255) characters and character classes.");
    v8_regexp_utils.shouldBeNull(core.RegExp.new("[Ā-ā]").firstMatch("a"));
    v8_regexp_utils.shouldBeNull(core.RegExp.new("[Ā]").firstMatch("a"));
    v8_regexp_utils.shouldBeNull(core.RegExp.new("Ā").firstMatch("a"));
    v8_regexp_utils.assertEquals(core.RegExp.new("[a]").firstMatch("a").group(0), "a");
    v8_regexp_utils.assertEquals(core.RegExp.new("[Ā-āa]").firstMatch("a").group(0), "a");
    v8_regexp_utils.assertEquals(core.RegExp.new("[Āa]").firstMatch("a").group(0), "a");
    v8_regexp_utils.assertEquals(core.RegExp.new("a").firstMatch("a").group(0), "a");
    v8_regexp_utils.assertEquals(core.RegExp.new("[a-Ā]").firstMatch("a").group(0), "a");
    v8_regexp_utils.assertEquals(core.RegExp.new("[Ā]").firstMatch("Ā").group(0), "Ā");
    v8_regexp_utils.assertEquals(core.RegExp.new("[Ā-ā]").firstMatch("Ā").group(0), "Ā");
    v8_regexp_utils.assertEquals(core.RegExp.new("Ā").firstMatch("Ā").group(0), "Ā");
  };
  dart.fn(extended$45characters$45match_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.extended$45characters$45match_test = extended$45characters$45match_test;
