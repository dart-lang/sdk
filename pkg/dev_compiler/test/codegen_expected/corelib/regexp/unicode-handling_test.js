dart_library.library('corelib/regexp/unicode-handling_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__unicode$45handling_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const unicode$45handling_test = Object.create(null);
  ...
  unicode$45handling_test.main = function() {
    v8_regexp_utils.description('Test for proper handling of Unicode RegExps and <a href="http://bugzilla.webkit.org/show_bug.cgi?id=7445">bug 7445</a>: Gmail puts wrong subject in replies.');
    let I3 = core.RegExp.new("^\\s*(fwd|re|aw|antw|antwort|wg|sv|ang|odp|betreff|betr|transf|reenv\\.|reenv|in|res|resp|resp\\.|enc|\\u8f6c\\u53d1|\\u56DE\\u590D|\\u041F\\u0435\\u0440\\u0435\\u0441\\u043B|\\u041E\\u0442\\u0432\\u0435\\u0442):\\s*(.*)$", {caseSensitive: false});
    let Ci = core.RegExp.new("\\s+");
    let BC = core.RegExp.new("^ ");
    let BG = core.RegExp.new(" $");
    function Gn(a) {
      return dart.dsend(dart.dsend(dart.dsend(a, 'replaceAll', Ci, " "), 'replaceAll', BC, ""), 'replaceAll', BG, "");
    }
    dart.fn(Gn, dynamicTodynamic());
    function cy(a) {
      let b = I3.firstMatch(core.String._check(a));
      if (b != null) {
        a = b.group(2);
      }
      return Gn(a);
    }
    dart.fn(cy, dynamicTodynamic());
    v8_regexp_utils.assertEquals(cy('Re: Moose'), 'Moose');
    v8_regexp_utils.assertEquals(cy('转发: Moose'), 'Moose');
    let sample = "sample bm⠠p cm\\u2820p";
    let inlineRe = core.RegExp.new(".m\\u2820p");
    v8_regexp_utils.assertEquals(inlineRe.firstMatch(sample).group(0), 'bm⠠p');
    let bsample = "sample bm|p cm\\u007cp";
    let binlineRe = core.RegExp.new(".m\\u007cp");
    v8_regexp_utils.assertEquals(binlineRe.firstMatch(bsample).group(0), 'bm|p');
  };
  dart.fn(unicode$45handling_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.unicode$45handling_test = unicode$45handling_test;
