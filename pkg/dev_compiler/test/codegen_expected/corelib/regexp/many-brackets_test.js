dart_library.library('corelib/regexp/many-brackets_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__many$45brackets_test(exports, dart_sdk, expect) {
  'use strict';
  const core = dart_sdk.core;
  const dart = dart_sdk.dart;
  const dartx = dart_sdk.dartx;
  const expect$ = expect.expect;
  const many$45brackets_test = Object.create(null);
  ...
  many$45brackets_test.main = function() {
    v8_regexp_utils.description('Test regular expression processing with many capturing brackets (200).');
    let count = 200;
    let regexp = "";
    ...
    let manyHellosArray = [];
    for (let i = 0; i <= count; ++i)
      manyHellosArray[dartx.add]("hello");
    let manyBracketsRegExp = core.RegExp.new(regexp);
    v8_regexp_utils.shouldBe(v8_regexp_utils.firstMatch('hello', manyBracketsRegExp), manyHellosArray);
  };
  dart.fn(many$45brackets_test.main, VoidTovoid$());
  ...
  // Exports:
  exports.many$45brackets_test = many$45brackets_test;
