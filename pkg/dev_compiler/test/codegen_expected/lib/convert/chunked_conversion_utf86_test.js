dart_library.library('lib/convert/chunked_conversion_utf86_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__chunked_conversion_utf86_test(exports, dart_sdk, expect) {
  'use strict';
  ...
  const chunked_conversion_utf86_test = Object.create(null);
  ...
  chunked_conversion_utf86_test.decode = function(bytes) {
    ...
    bytes[dartx.forEach](dart.fn(byte => {
      byteSink.add(JSArrayOfint().of([byte]));
    }, intTovoid()));
    byteSink.close();
    return buffer.toString();
  };
  dart.fn(chunked_conversion_utf86_test.decode, ListOfintToString());
  chunked_conversion_utf86_test.decodeAllowMalformed = function(bytes) {
    let buffer = new core.StringBuffer();
    let stringSink = convert.StringConversionSink.fromStringSink(buffer);
  ...
  dart.fn(chunked_conversion_utf86_test.decodeAllowMalformed, ListOfintToString());
  dart.defineLazy(chunked_conversion_utf86_test, {
    get TESTS() {
  ...
  chunked_conversion_utf86_test.main = function() {
    let allTests = chunked_conversion_utf86_test.TESTS[dartx.expand](dart.dynamic)(dart.fn(test => JSArrayOfListOfObject().of([JSArrayOfObject().of([test, "�"]), JSArrayOfObject().of([(() => {
    ...
    let allTests2 = chunked_conversion_utf86_test.TESTS2[dartx.map](ListOfObject())(dart.fn(test => {
    ...
    for (let test of (() => {
      ...
      expect$.Expect.throws(dart.fn(() => chunked_conversion_utf86_test.decode(bytes), VoidToString()), dart.fn(e => core.FormatException.is(e), dynamicTobool()));
      let expected = core.String._check(dart.dindex(test, 1));
      expect$.Expect.equals(expected, chunked_conversion_utf86_test.decodeAllowMalformed(bytes));
    }
  };
  dart.fn(chunked_conversion_utf86_test.main, VoidTodynamic());
  // Exports:
  exports.chunked_conversion_utf86_test = chunked_conversion_utf86_test;
});
