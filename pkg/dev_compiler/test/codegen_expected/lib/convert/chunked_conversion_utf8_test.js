dart_library.library('lib/convert/chunked_conversion_utf8_test', null, /* Imports */[
  'dart_sdk',
  'expect'
], function load__chunked_conversion_utf8_test(exports, dart_sdk, expect) {
  'use strict';
  ...
  const chunked_conversion_utf8_test = Object.create(null);
  ...
  chunked_conversion_utf8_test.decode = function(bytes, chunkSize) {
  ...
  dart.fn(chunked_conversion_utf8_test.decode, ListOfintAndintToString());
  chunked_conversion_utf8_test.main = function() {
    for (let test of unicode_tests.UNICODE_TESTS) {
      let bytes = dart.dindex(test, 0);
      let expected = dart.dindex(test, 1);
      expect$.Expect.stringEquals(core.String._check(expected), chunked_conversion_utf8_test.decode(ListOfint()._check(bytes), 1));
      expect$.Expect.stringEquals(core.String._check(expected), chunked_conversion_utf8_test.decode(ListOfint()._check(bytes), 2));
      expect$.Expect.stringEquals(core.String._check(expected), chunked_conversion_utf8_test.decode(ListOfint()._check(bytes), 3));
      expect$.Expect.stringEquals(core.String._check(expected), chunked_conversion_utf8_test.decode(ListOfint()._check(bytes), 4));
    }
  };
  dart.fn(chunked_conversion_utf8_test.main, VoidTodynamic());
  ...
  // Exports:
  exports.chunked_conversion_utf8_test = chunked_conversion_utf8_test;
