dart_library.library('lib/convert/streamed_conversion_json_utf8_encode_test', null, /* Imports */[
  'dart_sdk',
  'async_helper',
  'expect'
], function load__streamed_conversion_json_utf8_encode_test(exports, dart_sdk, async_helper, expect) {
  'use strict';
  ...
  const streamed_conversion_json_utf8_encode_test = Object.create(null);
  ...
  let ObjectToStreamOfListOfint = () => (ObjectToStreamOfListOfint = dart.constFn(dart.definiteFunctionType(StreamOfListOfint(), [core.Object])))();
  ...
  dart.defineLazy(streamed_conversion_json_utf8_encode_test, {
    get JSON_UTF8() {
      return convert.JSON.fuse(convert.UTF8);
    }
  });
  streamed_conversion_json_utf8_encode_test.encode = function(o) {
    let controller = null;
    controller = async.StreamController.new({onListen: dart.fn(() => {
        dart.dsend(controller, 'add', o);
        dart.dsend(controller, 'close');
      }, VoidTovoid())});
    return StreamOfListOfint()._check(dart.dsend(dart.dload(controller, 'stream'), 'transform', streamed_conversion_json_utf8_encode_test.JSON_UTF8.encoder));
  };
  dart.fn(streamed_conversion_json_utf8_encode_test.encode, ObjectToStreamOfListOfint());
  streamed_conversion_json_utf8_encode_test.testUnpaused = function(expected, stream) {
  ...
  dart.fn(streamed_conversion_json_utf8_encode_test.testUnpaused, ListOfintAndStreamTovoid());
  streamed_conversion_json_utf8_encode_test.testWithPauses = function(expected, stream) {
    async_helper$.asyncStart();
    let accumulated = [];
    let sub = null;
    sub = stream.listen(dart.fn(x => {
      accumulated[dartx.addAll](core.Iterable._check(x));
      dart.dsend(sub, 'pause', async.Future.delayed(core.Duration.ZERO));
    }, dynamicTovoid()), {onDone: dart.fn(() => {
        expect$.Expect.listEquals(expected, accumulated);
        async_helper$.asyncEnd();
      }, VoidTovoid())});
  };
  dart.fn(streamed_conversion_json_utf8_encode_test.testWithPauses, ListOfintAndStreamTovoid());
  streamed_conversion_json_utf8_encode_test.main = function() {
    for (let test of core.Iterable._check(json_unicode_tests.JSON_UNICODE_TESTS)) {
      let expected = dart.dindex(test, 0);
      let object = dart.dindex(test, 1);
      streamed_conversion_json_utf8_encode_test.testUnpaused(ListOfint()._check(expected), streamed_conversion_json_utf8_encode_test.encode(object));
      streamed_conversion_json_utf8_encode_test.testWithPauses(ListOfint()._check(expected), streamed_conversion_json_utf8_encode_test.encode(object));
    }
  };
  dart.fn(streamed_conversion_json_utf8_encode_test.main, VoidTovoid());
  ...
  // Exports:
  exports.streamed_conversion_json_utf8_encode_test = streamed_conversion_json_utf8_encode_test;
