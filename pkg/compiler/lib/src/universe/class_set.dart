...
class ClassHierarchyNode {
  ...
  /// function stops immediately. If [ForEach.SKIP_CHILDREN] is returned, the
  ...
  ForEach forEachSubclass(
    ...
    if (!mask.contains(Instantiation.UNINSTANTIATED) && !isInstantiated) {
      // Bypass children since all are uninstantiated.
      return forEach;
    }
...
class ClassSet {
  ...
  ForEach forEachSubtype(
      ...
      EnumSet<Instantiation> mask,
      {bool strict: false}) {
    if (!mask.contains(Instantiation.UNINSTANTIATED) && !node.isInstantiated) {
      // Bypass children since all are uninstantiated.
...
enum ForEach {
  ...
  STOP,
  /// Iteration skips the child classes of the current class.
  SKIP_CHILDREN,
}
