...
class StackFrame {
  ...
  final Map<ir.Variable, HInstruction> letBindings;
...
class KernelSsaGraphBuilder extends ir.VisitorDefault<void>
  ...
  Map<ir.Variable, HInstruction> get _letBindings => _currentFrame!.letBindings;
  ...
  ir.FunctionNode _ensureDefaultArgumentValues(ir.FunctionNode function) {
    ...
    // actually used.
    void registerDefaultValue(ir.FunctionParameter node) {
    ...
    for (ir.Constructor body in constructorData.constructorChain.reversed) {
      ...
      _inlinedFrom(
        ...
        () {
          ...
          void handleParameter(ir.Variable node, {required bool isElided}) {
            if (isElided) return;
  ...
  void _buildInitializers(
    ...
    for (var initializer in constructor.initializers) {
      if (initializer is ir.FieldInitializer) {
        ...
        ir.Variable variable = initializer.variable;
  ...
  void _inlineSuperOrRedirectCommon(
    ...
    _inlinedFrom(
      ...
      () {
        void handleParameter(ir.FunctionParameter node) {
          Local parameter = _localsMap.getLocalVariable(node);
  ...
  void _potentiallyAddFunctionParameterTypeChecks(
    ...
    void handleParameter(ir.FunctionParameter parameter) {
      Local local = _localsMap.getLocalVariable(parameter);
      ...
      DartType type = _getDartTypeIfValid(parameter.type);
  ...
  void _buildExternalNativeFunctionNode(
    ...
    void handleParameter(ir.FunctionParameter param) {
  ...
  void _openFunction(
    ...
    List<ir.Variable> elidedParameters = [];
    ...
    if (memberContextNode != null) {
      for (ir.Variable node in elidedParameters) {
        Local local = _localsMap.getLocalVariable(node);
  ...
  void visitVariableGet(ir.VariableGet node) {
    ir.Variable variable = node.variable;
  ...
  List<HInstruction> _visitArgumentsForStaticTarget(
    ...
    while (values.length < parameterStructure.positionalParameters) {
      ir.PositionalParameter parameter =
  ...
  HInstruction _defaultValueForParameter(ir.Variable parameter) {
