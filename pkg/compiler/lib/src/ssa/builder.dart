...
class KernelSsaGraphBuilder extends ir.VisitorDefault<void>
  ...
  void _inlinedFrom(MemberEntity inlinedTarget,
      SourceInformation? callSourceInformation, void Function() f) {
  ...
  void visitCheckLibraryIsLoaded(ir.CheckLibraryIsLoaded node) {
    ...
    String loadId = closedWorld.outputUnitData.getImportDeferName(
        _elementMap.getSpannable(targetElement, node), import);
      ...
      if (!_isReachable) {
        ...
        if (stack.isNotEmpty) {
          reporter.internalError(
              noLocationSpannable, 'Non-empty instruction stack.');
        }
        return;
      }
    }
    ...
    if (function.isExternal &&
      ...
      if (node.isConst) {
        ...
        stack.add(graph.addConstant(
            _elementMap.getConstantValue(node)!, closedWorld,
            sourceInformation: sourceInformation));
      } else {
  ...
  js.Name? _getNameForJsGetName(ConstantValue constant, ModularNamer namer) {
    ...
    return namer.getNameForJsGetName(
        currentElementSpannable, JsGetName.values[index]);
  }
  ...
  js.Template? _templateForBuiltin(JsBuiltin builtin) {
    switch (builtin) {
      ...
      case JsBuiltin.isJsInteropTypeArgument:
        reporter.internalError(
            noLocationSpannable, "Unhandled Builtin: $builtin");
    }
  }
  ...
  bool _doTryInlineMethod(
    ...
    bool heuristicSayGoodToGo() {
      ...
      if (insideLoop) {
        maxInliningNodes = InlineWeeder.inliningNodesInsideLoop +
            InlineWeeder.inliningNodesInsideLoopArgFactor * numParameters;
      } else {
        maxInliningNodes = InlineWeeder.inliningNodesOutsideLoop +
