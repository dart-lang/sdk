...
To launch a _Web Worker_, the pre-compiled wasm worker and associated
_DartPad SDK_ assets must be created using `dart run dartpad setup`. You must
host the generated directory and provide its URL to
`DartPadSdk(assetBaseUrl: myUri)`.
...
Future<void> main() async {
  final sdk = DartPadSdk(
    // URL where the DartPad SDK assets generated by `dart run dartpad setup`
    // are hosted.
    assetBaseUrl: Uri.base.resolve('dartpad_sdk/'),
  );
