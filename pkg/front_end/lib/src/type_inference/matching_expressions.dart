...
class MatchingExpressionVisitor
  ...
  DelayedExpression visitAndPattern(
    ...
    return new DelayedAndExpression(
      visitPattern(node.left, matchedExpression),
  ...
  DelayedExpression visitAssignedVariablePattern(
    ...
    return new DelayedAssignment(
      ...
      valueExpression,
      fileOffset: node.fileOffset,
      hasEffect: node.hasObservableEffect,
    );
  }
  ...
  DelayedExpression visitCastPattern(
    ...
    return new EffectExpression(
      asExpression,
      visitPattern(node.pattern, asExpression),
    );
  }
  ...
  DelayedExpression visitNullAssertPattern(
    ...
    CacheableExpression nullAssertExpression = matchingCache
        .createNullAssertMatcher(
          matchedExpression,
          fileOffset: node.fileOffset,
        );
    return new EffectExpression(
      nullAssertExpression,
      visitPattern(node.pattern, nullAssertExpression),
    );
  }
  ...
  DelayedExpression visitObjectPattern(
    ...
    for (NamedPattern field in node.fields) {
      ...
      CacheableExpression objectExpression = matchingCache
          .createPropertyGetExpression(
            typedMatchedExpression,
            field.fieldName.text,
            expression,
  ...
  DelayedExpression visitOrPattern(
    ...
    matchingCache.declareJointVariables(
      node.orPatternJointVariables,
      node.left.declaredVariables,
      node.right.declaredVariables,
    );
  ...
  DelayedExpression visitRecordPattern(
    ...
    for (Pattern fieldPattern in node.patterns) {
      ...
      if (fieldPattern is NamedPattern) {
        ...
        assert(
          node.requiredType!.named.any(
            (named) => named.name == fieldPattern.name,
          ),
        );
      } else {
      ...
      DelayedExpression fieldMatcher = visitPattern(
        fieldPattern,
        fieldExpression,
      );
      matchingExpression = DelayedAndExpression.merge(
  ...
  DelayedExpression visitWildcardPattern(
    ...
    if (node.type != null) {
      return new DelayedIsExpression(
        matchedExpression,
        node.type!,
        fileOffset: node.fileOffset,
      );
