...
class ConstantEvaluator
  ...
  Constant visitRecordIndexGet(RecordIndexGet node) {
    final Constant receiver = _evaluateSubexpression(node.receiver);
    if (receiver is AbortConstant) return receiver;
    // No enableConstFunctions guard: RecordIndexGet nodes are generated by
    // record spread desugaring in normal const contexts, not only by the
    // const-functions experiment.
  ...
  Constant visitRecordNameGet(RecordNameGet node) {
    final Constant receiver = _evaluateSubexpression(node.receiver);
    if (receiver is AbortConstant) return receiver;
    // No enableConstFunctions guard: RecordNameGet nodes are generated by
    // record spread desugaring in normal const contexts, not only by the
    // const-functions experiment.
    if (receiver is RecordConstant) {
