...
class ExceptionHandlerStack {
  ...
  /// Maps Wasm `try_table` blocks to number of handlers in [_handlers] that they
  /// cover for.
  ...
  /// instructions.
  final List<
    ({w.Label afterTryBlock, w.Label catchJumpLabel, w.Label? catchJsJumpLabel})
  >
  _tryBlockLabels = [];
  ...
  /// Generates Wasm `try_table` blocks for Dart `try` blocks wrapping the current
  /// CFG block.
  ///
  ...
  void _generateTryBlocks(w.InstructionsBuilder b) {
    ...
    final catchJumpLabel = b.block([], [
      codeGen.translator.topTypeNonNullable,
      codeGen.translator.stackTraceType,
    ]);

    // Generate a catch clause to catch JS exceptions if any of the covered
    // handlers can catch JS exceptions.
    w.Label? catchJsJumpLabel;
    if (!codeGen.translator.options.standalone) {
      for (int i = 0; i < handlersToCover; i++) {
        if (_handlers[_handlers.length - 1 - i].canHandleJSExceptions) {
          catchJsJumpLabel = b.block([], [w.RefType.extern(nullable: true)]);
          break;
        }
      }
    }
    ...
    _tryBlockLabels.add((
      afterTryBlock: afterTryBlock,
      catchJumpLabel: catchJumpLabel,
      catchJsJumpLabel: catchJsJumpLabel,
    ));
  }

  /// Terminates Wasm `try_table` blocks generated by [_generateTryBlocks].
  ///
  ...
  void _terminateTryBlocks() {
    ...
    for (final int nCoveredHandlers in _tryBlockNumHandlers.reversed) {
      ...
      final (:afterTryBlock, :catchJumpLabel, :catchJsJumpLabel) =
...
class Finalizer extends _ExceptionHandler {
  ...
  @override
  bool get canHandleJSExceptions => !codeGen.translator.options.standalone;
