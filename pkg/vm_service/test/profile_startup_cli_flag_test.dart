...
import 'profile_startup_cli_flag_lib.dart' as testee_lib;

Future<void> main([args = const <String>[]]) async {
  await IsolateTestHarness(
    'profile_startup_cli_flag_lib.dart',
    args,
  )
      .hasStoppedAtExit()
    ...
    // [IsolateTestHarness.run] below, and [testeeMain] spins for 10 seconds, so the
    ...
    final timeExtentNanos = computeTimeExtentNanos(
      packets,
      timeOriginNanos,
    );
    print(
    ...
    final filteredTrace = Trace.fromBuffer(
      base64Decode(filteredResult.samples!),
    );
    final filteredPackets = filteredTrace.packet;
    final filteredTraceTimeOriginNanos = computeTimeOriginNanos(
      filteredPackets,
    );
    ...
    final secondUnfilteredResult = await service.getPerfettoCpuSamples(
      isolateRef.id!,
    );
    final secondUnfilteredTrace = Trace.fromBuffer(
      base64Decode(secondUnfilteredResult.samples!),
    );
    expect(
      extractPerfSamplesFromTracePackets(
        secondUnfilteredTrace.packet,
      ).length,
      perfSamples.length,
    );
  }).run(
