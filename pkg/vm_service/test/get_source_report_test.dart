...
import 'get_source_report_lib.dart' as testee_lib;
...
void main([args = const <String>[]]) =>
    IsolateTestHarness('get_source_report_lib.dart', args)
        .hasStoppedAtBreakpoint()
        .stoppedAtLine('LINE_A')
        .addCustomTestWithParser((VmService service, IsolateRef isolateRef,
      ...
      final expectedRange = SourceReportRange(
        scriptIndex: 0,
        startPos: parser.offsetForTag('OFFSET_START'),
        endPos: parser.offsetForTag('OFFSET_END'),
        compiled: true,
        coverage: SourceReportCoverage(
          hits: [
            parser.offsetForTag('OFFSET_START'),
            parser.offsetForTag('OFFSET_COMPARE'),
            parser.offsetForTag('OFFSET_PRINT_POSITIVE'),
            parser.offsetForTag('OFFSET_DEBUGGER'),
          ],
          misses: [parser.offsetForTag('OFFSET_PRINT_NEGATIVE')],
        ),
      );
      ...
      expect(ranges.length, greaterThanOrEqualTo(7));
      ...
      expect(scripts[0].uri!, endsWith('get_source_report_lib.dart'));
