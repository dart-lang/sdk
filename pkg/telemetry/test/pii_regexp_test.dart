...
void main() {
  group('filterPiiFromErrorMessage', () {
    test('filters paths greedily', () {
      expect(
        filterPiiFromErrorMessage(r'a message: C:\My\Special Path'),
      ...
      expect(
        filterPiiFromErrorMessage(r'a message: Q:\I Have A Lot of Drives'),
        equals('a message: <path>'),
      );
      expect(
        filterPiiFromErrorMessage(r'a message: R:\'),
        equals('a message: <path>'),
      );
      expect(
      ...
      expect(
        filterPiiFromErrorMessage(r'a message: ../relative_path/to_some/dart'),
        equals('a message: <path>'),
      );
      expect(
        filterPiiFromErrorMessage(
    ...
    test('filters filenames without path separators without being greedy', () {
      expect(
        filterPiiFromErrorMessage(r'mine.dart: a special file'),
        equals('<filename>: a special file'),
      );
      expect(
        filterPiiFromErrorMessage(r'foo.js: a special file'),
        equals('<filename>: a special file'),
      );
      expect(
        filterPiiFromErrorMessage(r'something.exe: a special file'),
        equals('<filename>: a special file'),
      );
    });
    test('does not filter things that should not match', () {
      expect(
        filterPiiFromErrorMessage('User ran a Commodore 64 executable'),
        equals('User ran a Commodore 64 executable'),
      );
      expect(
        filterPiiFromErrorMessage('franchiseError: somehow Palpatine returned'),
