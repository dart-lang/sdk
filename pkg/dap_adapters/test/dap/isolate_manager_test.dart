  ...
  group('IsolateManager', () {
    ...
    test('sends only changes to SDK libraries debuggable flag', () async {
      ...
      expect(
        ...
        isNot(contains(startsWith('setLibraryDebuggable(isolate1, libSdk,'))),
      );
    ...
    test(
      'sends only changes to external package libraries debuggable flag',
      () async {
    ...
    test(
      'never sends values for local package libraries debuggable flag',
      () async {
        isolateManager.debugSdkLibraries = true;
        isolateManager.debugExternalPackageLibraries = true;
        await isolateManager.applyDebugOptions();
    ...
    test('uses uri converter to convert resolved uri', () async {
      String? receivedUri;
      adapter.setUriConverter((String? uri) {
        receivedUri = uri;
        return '/returned/file/path.dart';
      });

      adapter.mockService.lookupResolvedPackageUrisResponse = UriList(
        uris: ['scheme://file/path.dart'],
      );

      final thread = isolateManager.threads[0];
      final resolved = await thread.resolveUriToPath(
        Uri.parse('package:pkg/path.dart'),
      );
    ...
    test('sends resume after kPausePostRequest', () async {
      ...
      await adapter.isolateManager.handleEvent(
        Event(
      ...
      expect(
        adapter.mockService.calledMethods.map((cm) => cm.method),
