...
class MockVmService implements VmService {
  ...
  final externalPackageLibrary = LibraryRef(
    id: 'libPkgExternal',
    uri: 'package:external/foo.dart',
  );
  final localPackageLibrary = LibraryRef(
    id: 'libPkgLocal',
    uri: 'package:local/foo.dart',
  );
  ...
  Future<VM> getVM() async {
    return VM(isolates: [isolate1, isolate2]);
  }
  ...
  Future<Response> callMethod(
    ...
    if (method == 'getDartDevelopmentServiceVersion') {
      ...
      isolateId ??= args != null ? args['isolateId'] as String? : null;
      return {isolate1.id, isolate2.id}.contains(isolateId)
          ? Success()
