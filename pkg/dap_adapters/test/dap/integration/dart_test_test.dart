...
main() {
  ...
  group('dart test', () {
    ...
    void expectStandardSimpleTestResults(TestEvents events) {
      ...
      // package:test.
      final eventNames = events.testNotifications
          .map((e) => e['type'])
          .toList();
      ...
      expect(eventNames, containsAllInOrder(['start', 'allSuites']));
      ...
      final outputEvents = await client.collectTestOutput(
        ...
        start: () => client
            .hitBreakpoint(
              ...
              args: [
                '--chain-stack-traces',
              ], // to suppress warnings in the output
            )
    ...
    test('responds to setBreakpoints before any breakpoint events', () async {
      ...
      final testFile = dap.createTestFile(
        simpleTestBreakpointProgramWith50ExtraLines,
      );
