...
class MyClass {}
    ''');
      ...
      await client.expectScopeVariables(topFrameId, 'Globals', '''
...
class MyClass {
  final myField = 'myFieldValue';
    ...
    ''');
      ...
      final stop = await client.hitBreakpoint(testFile, breakpointLine);
      final expectedVariable = await client.getGlobalVariable(
        stop.threadId!,
        'globalMyClass',
      );

      // Check the child fields.
      return client.expectVariables(expectedVariable.variablesReference, '''
...
void main(List<String> args) {
  throw 'my error';
    ...
    ''');
      ...
      // exception.
      await client.expectScopeVariables(topFrameId, 'Exceptions', r'''
...
void main(List<String> args) {
  ...
  print('Hello!'); $breakpointMarker
    ...
    test(
      'includes eager public getters when evaluateGettersInDebugViews=true',
      () async {
        final client = dap.client;
        final testFile = dap.createTestFile('''
void main(List<String> args) {
  ...
  String get _privateString => '222';
    ...
    ''');
      ...
      final stop = await client.hitBreakpoint(
        ...
        breakpointLine,
        launch: () =>
            client.launch(testFile.path, showGettersInDebugViews: true),
      );
      ...
      final namedRecordVariable = variables.variables.singleWhere(
        (variable) => variable.name == 'publicString',
      );
      expect(namedRecordVariable.variablesReference, isPositive);
      await client.expectVariables(namedRecordVariable.variablesReference, r'''
...
void main(List<String> args) {
  ...
  print('Hello!'); $breakpointMarker
    ...
    ''');
      ...
      final namedRecordVariable = variables.variables.singleWhere(
        (variable) => variable.name == 'namedRecord',
      );
      expect(namedRecordVariable.variablesReference, isPositive);
      await client.expectVariables(namedRecordVariable.variablesReference, r'''
...
void main(List<String> args) {
  ...
  print('Hello!'); $breakpointMarker
    ...
    ''');
      ...
      await client.expectVariables(variableOne.variablesReference, '''
            key: "one"
...
void main(List<String> args) {
  ...
  print('Hello!'); $breakpointMarker
    ...
    ''');
      ...
      await client.expectVariables(keyVariable.variablesReference, '''
            isUtc: false
        ''');
      ...
      await client.expectVariables(valueVariable.variablesReference, '''
            message: "my error"
        ''');
    });
...
void main() {
  ...
  print('Done!'); $breakpointMarker
    ...
    ''');
          ...
          final inspectWrapper = await client.getValidVariables(
            outputEvent.variablesReference!,
          );
      ...
      checkInspect('"My String"', expectedVariables: 'String: "My String"');

      checkInspect('null', expectedVariables: 'Null: null');
...
void main(List<String> args) {
  ...
  calloc.free(ptr);
    ...
    ''');
        ...
        final resolvedVars = await client.getValidVariables(
          rawBytesVar.variablesReference,
        );
        ...
        await client.expectVariables(resolvedVar.variablesReference, '''
      [0]: 0xde
      [1]: 0xad
      [2]: 0xbe
      [3]: 0xef
