  ...
  group('debug mode', () {
    ...
    test('runs a simple script', () async {
      ...
      final outputEvents = await dap.client.collectOutput(
        launch: () => dap.client.launch(testFile.path, args: ['one', 'two']),
      );
    ...
    test('runs a simple script using runInTerminal request', () async {
      ...
      Process? proc;
      dap.client.handleRequest('runInTerminal', (args) async {
    ...
    test(
      'does not resume isolates if user passes --pause-isolates-on-start',
      () async {
    ...
    group('progress notifications', () {
      ...
      test('sends standard events when supported', () async {
        ...
        final standardEventsFuture = dap.client
            .standardProgressEvents()
            .toList();
        ...
        expect(standardEvents.map((e) => e.event), [
          'progressStart',
          'progressUpdate',
          'progressEnd',
        ]);
      ...
      test('sends custom events when requested', () async {
        ...
        await Future.wait([
          dap.client.event('terminated'),
          dap.client.initialize(),
          dap.client.launch(testFile.path, sendCustomProgressEvents: true),
        ], eagerError: true);
        ...
        expect(customEvents.map((e) => e.event), [
          'dart.progressStart',
          'dart.progressUpdate',
          'dart.progressEnd',
        ]);
    ...
    test('can hot reload', () async {
      ...
      await dap.client.outputEvents.firstWhere(
        (event) => event.output.trim() == originalText,
      );
      ...
      // Expect the new text.
      await dap.client.outputEvents.firstWhere(
        (event) => event.output.trim() == newText,
      );
    ...
    test('can pause', () async {
      ...
      await Future.wait([
        ...
        dap.client.outputEvents.firstWhere(
          (event) => event.output.contains('Looping'),
        ),
      ]);
    ...
    test('can restart frame', () async {
      ...
      expect(outputMessages, containsAll(['Hello', 'Hello', 'World']));
    });
    ...
    test('resolves URIs in tool events to file:///', () async {
      final client = dap.client;
      final testFile = dap.createTestFile(
        simpleToolEventWithDartCoreUriProgram,
      );
