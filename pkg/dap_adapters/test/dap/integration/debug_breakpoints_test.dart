...
main() {
  ...
  group('debug mode breakpoints', () {
    ...
    test('resolves modified breakpoints', () async {
      ...
      final breakpointResolveSubscription = client.breakpointChangeEvents
          .listen((event) {
      ...
      for (var i = 1; i <= 4; i++) {
        ...
        final response = await client.setBreakpoints(
          testFile,
          breakpointLinesToSend,
        );
    ...
    test('provides reason for failed breakpoints', () async {
      ...
      final breakpointChangedSubscription = client.breakpointChangeEvents
          .listen((event) {
            breakpointReasons.add(
              '${event.breakpoint.reason}: ${event.breakpoint.message}',
            );
          });
    ...
    test('provides reason for not-yet-resolved breakpoints', () async {
      ...
      expect(
        bps.breakpoints.single.message,
...
void main(List<String> args) async {
  ...
  print('Hello!'); $breakpointMarker
    ...
    test('does not fail updating breakpoints after a removal '
        'if two breakpoints resolved to the same location', () async {
      final client = dap.client;
      final testFile = dap.createTestFile(
        simpleBreakpointWithLeadingBlankLineProgram,
      );
      final breakpointLine = lineWith(testFile, breakpointMarker);
    ...
    test(
      'stops at a line breakpoint in the SDK set via local sources',
      () async {
        final client = dap.client;
        final testFile = dap.createTestFile(simpleBreakpointProgram);
...
void main(List<String> args) async {
  ...
  print('Hello!'); $stepMarker
    ...
    test('stops at a line breakpoint and can step over (next) '
...
void main(List<String> args) async {
  ...
  print('Hello!'); $stepMarker
    ...
    test('ignores resume request for an exited isolate', () async {
      ...
      final threadExitFuture = client.threadEvents
          .where((event) => event.reason == 'exited')
          .first;
      await Future.wait([
        threadExitFuture,
        client.initialize(),
  ...
  foo();
    ...
    ''');
      ...
      final stop = await dap.client.hitBreakpoint(
        ...
        breakpointLine,
        launch: () =>
            client.launch(testFile.path, debugExternalPackageLibraries: true),
      );
...
void main(List<String> args) async {
  ...
  print('Hello!'); $stepMarker
    ...
    ''');
      ...
      await client.custom('updateDebugOptions', {'debugSdkLibraries': false});
    ...
    test('handles breakpoints correctly in newly spawned isolates', () async {
      ...
      final client = dap.client;
      final testFile = dap.createTestFile(
        multiIsolateBreakpointResolutionProgram,
      );
  ...
  group('debug mode conditional breakpoints', () {
    test('stops with condition evaluating to true', () async {
      ...
      await client.hitBreakpoint(testFile, breakpointLine, condition: '1 == 1');
    });
  ...
  group('debug mode logpoints', () {
    ...
    Future<void> testLogPoint(
      ...
      expect(outputMessages, contains(expectedMessage));
    }
