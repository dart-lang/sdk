...
import 'package:dap_adapters/src/adapters/dart.dart';
...
void main(List<String> args) {
  ...
  print('Hello!'); $breakpointMarker
    ...
    test('evaluates expressions with complex results', () async {
      ...
      await client.expectVariables(result.variablesReference, '''
...
void foo() {
  var a = 111; $breakpointMarker
    ...
    test('returns the full message for evaluation errors', () async {
      ...
      final topFrameId = await client.getTopFrameId(stop.threadId!);
      expectResponseError(
        client.evaluate('1 + "a"', frameId: topFrameId),
        allOf([
    ...
    test(
      'returns short errors for evaluation in "watch" context (rpc error)',
      () async {
        final client = dap.client;
        final testFile = dap.createTestFile(simpleBreakpointProgram);
        final breakpointLine = lineWith(testFile, breakpointMarker);
    ...
    test(
      'returns short errors for evaluation in "watch" context (ErrorRef)',
      () async {
        final client = dap.client;
        final testFile = dap.createTestFile(simpleBreakpointProgram);
        final breakpointLine = lineWith(testFile, breakpointMarker);
    ...
    test(
      'returns whole string without quotes when context is "clipboard"',
      () async {
        final client = dap.client;
        final testFile = dap.createTestFile(simpleBreakpointProgram);
        final breakpointLine = lineWith(testFile, breakpointMarker);
    ...
    test('evaluation service zones are invalidated on resume', () async {
      ...
      expect(await loggedEvaluateInFrameRequests, isNotEmpty);
    });
    ...
    group('global evaluation', () {
      test(
        'can evaluate in a bin/ file when not paused given a bin/ URI',
        () async {
          final client = dap.client;
      ...
      test('returns a suitable error with no context', () async {
        const expectedErrorMessage =
            'Evaluation is only supported when the '
        ...
        final response = await client.sendRequest(
          EvaluateArguments(expression: 'myGlobal'),
          allowFailure: true,
        );
      ...
      test('returns a suitable error with an unknown script context', () async {
        ...
        final context = Uri.file(
          testFile.path.replaceAll('.dart', '_invalid.dart'),
        ).toString();
        final response = await client.sendRequest(
          EvaluateArguments(expression: 'myGlobal', context: context),
          allowFailure: true,
        );
        expect(response.success, isFalse);
        expect(
