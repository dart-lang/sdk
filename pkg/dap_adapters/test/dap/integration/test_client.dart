...
class DapTestClient {
  ...
  Future<Response> source(Source source) => sendRequest(
    SourceArguments(source: source, sourceReference: source.sourceReference!),
  );
  ...
  /// [launch] method.
  Future<void> start({File? file, Future<Object?> Function()? launch}) {
    return Future.wait([
      initialize(),
...
extension DapTestClientExtension on DapTestClient {
  ...
  Future<StoppedEventBody> hitBreakpoint(
    ...
    assert(
      condition == null || additionalBreakpoints == null,
  ...
  Future<SetBreakpointsResponseBody> setBreakpoint(
    File file,
    int line, {
    String? condition,
  }) async {
  ...
  Future<bool> get ddsAvailable async {
    ...
    final protocolList = vm.ProtocolList.parse(
      response.body as Map<String, Object?>?,
    );
  ...
  Future<StoppedEventBody> expectStop(
    ...
    final result = await getValidStack(
      stop.threadId!,
      startFrame: 0,
      numFrames: 1,
    );
  ...
  Future<StackTraceResponseBody> getValidStack(
    ...
    final response = await stackTrace(
      threadId,
      startFrame: startFrame,
      numFrames: numFrames,
    );
  ...
  Future<VariablesResponseBody> expectVariables(
    ...
    final expectedLines = expectedVariables
        .trim()
        .split('\n')
        .map((l) => l.trim())
        .toList();
    ...
    if (ignore != null) {
      ...
      for (final ignored in ignore) {
        expect(
          ...
          isTrue,
          reason:
              'Variable "$ignored" should be ignored but was '
  ...
  Future<int> getTopFrameId(int threadId) async {
    final stack = await getValidStack(threadId, startFrame: 0, numFrames: 1);
  ...
  Future<EvaluateResponseBody> expectEvalResult(
    ...
    final body = EvaluateResponseBody.fromJson(
      response.body as Map<String, Object?>,
    );
