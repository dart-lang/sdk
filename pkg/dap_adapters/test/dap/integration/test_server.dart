...
import 'package:dap_adapters/src/server.dart';
...
class InProcessDapTestServer extends DapTestServer {
  ...
  static Future<InProcessDapTestServer> create({
    ...
    return InProcessDapTestServer._([...?additionalArgs], onError: onError);
  }
}
...
class OutOfProcessDapTestServer extends DapTestServer {
  ...
  static Future<OutOfProcessDapTestServer> create({
    ...
    final ddsEntryScript = await Isolate.resolvePackageUri(
      Uri.parse('package:dds/dds.dart'),
    );
    final ddsLibFolder = path.dirname(ddsEntryScript!.toFilePath());
    final dartdevScript = path.normalize(
      path.join(ddsLibFolder, '../../dartdev/bin/dartdev.dart'),
    );
