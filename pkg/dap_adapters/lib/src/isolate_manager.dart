...
class IsolateManager {
  ...
  final _terseBreakpointFailureRegex = RegExp(
  ...
  Future<void> setBreakpoints(
    ...
    await Future.wait(
      _threadsByThreadId.values.map(
        (thread) => _sendBreakpoints(thread, uri: uri),
      ),
    );
  }
  ...
  Future<bool> _breakpointConditionEvaluatesTrue(
    ...
    final result = await _evaluateAndPrintErrors(
      thread,
      condition,
      'condition',
    );
  ...
  void queueFailedBreakpointEvent(
    ...
    var userMessage = error is vm.RPCError
        ...
        : error.toString();
    final terseMessageMatch = _terseBreakpointFailureRegex.firstMatch(
      userMessage,
    );
  ...
  Future<List<Uri?>?> _lookupResolvedPackageUris<T extends vm.Response>(
    ...
    try {
      final res = await _adapter.vmService?.lookupResolvedPackageUris(
        isolateId,
        uriStrings,
        local: true,
      );
    ...
    final results = await Future.wait(
      messages.map((message) {
        ...
        final expression = jsonEncode(message)
            ...
            .replaceAllMapped(
              _braceNotPrefixedByDollarOrBackslashPattern,
              (match) => '${match[1]}\${',
            )
  ...
  Future<void> _sendBreakpoints(ThreadInfo thread, {String? uri}) async {
    ...
    for (final uri in uris) {
      ...
      final existingBreakpointsForIsolate = _vmBreakpointsByIsolateIdAndUri
          .putIfAbsent(isolateId, () => {});
      final existingBreakpointsForIsolateAndUri = existingBreakpointsForIsolate
          .putIfAbsent(uri, () => {});
      ...
      final breakpointsToRemove = existingBreakpointsForIsolateAndUri.values
          .toList();
    ...
    await Future.wait(
      libraries.map((library) async {
        ...
        final isDebuggableNew = libraryUri != null
            ...
            : false;
        final isDebuggableCurrent = thread.getIsLibraryCurrentlyDebuggable(
          library,
        );
  ...
  Future<bool> _shouldHitBreakpoint(
    ...
    // hit the breakpoint.
    final clientBreakpointsWithConditions = breakpoints
        .where((bp) => bp?.condition?.isNotEmpty ?? false)
        .toList();
    ...
    // Otherwise, we need to evaluate all of the conditions and see if any are
    // true, in which case we will also hit.
    final conditions = clientBreakpointsWithConditions
        .map((bp) => bp!.condition!)
        .toSet();
...
class ThreadInfo with FileUtils {
  ...
  Future<Uri?> resolvePathToUri(
    ...
    final packageUriList = performResolve
        ? await _manager._adapter.vmService?.lookupPackageUris(isolate.id!, [
            uri.toString(),
          ])
        : null;
  ...
  Future<List<Uri?>> resolveUrisToPathsBatch(List<Uri> uris) async {
    ...
    if (requiredUris.isNotEmpty) {
      ...
      try {
        final results = await _manager._lookupResolvedPackageUris(
          isolate,
          requiredUris,
        );
        if (results == null) {
          ...
          final reason = results.length > requiredUris.length
              ? 'more'
              : 'fewer';
          final message =
          ...
          completers.forEach(
            (uri, completer) => completer.completeError(error),
          );
        } else {
  ...
  Uri? _convertPathToGoogle3Uri(Uri input) {
    ...
    if (inputPath.startsWith('/google') && inputPath.contains(search)) {
      ...
      return Uri(scheme: 'google3', host: '', path: remainingPath);
    }
  ...
  Future<void> clearTemporaryData() async {
    ...
    if (_currentEvaluationZoneIdCompleter != null) {
      ...
      if (evalZoneId != null) {
        await _manager._adapter.vmService?.invalidateIdZone(
          isolate.id!,
          evalZoneId,
        );
      }
  ...
  Future<vm.LibraryRef?> getLibraryForFileUri(Uri scriptFileUri) async {
    ...
    final scriptRef = scriptRefs.singleWhereOrNull(
      (script) => candidateUris.contains(script.uri),
    );
