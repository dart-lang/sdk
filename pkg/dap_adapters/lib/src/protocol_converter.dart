...
class ProtocolConverter {
  ...
  Future<List<dap.Variable>> convertVmInstanceToVariablesList(
    ...
    if (instance.kind == vm.InstanceKind.kPointer) {
      ...
      return Future.wait(
        elements.cast<vm.Response?>().mapIndexed((index, response) {
          final name = '[${start + index}]';
          final nameForEvaluation = instance.isSet
              ? '.elementAt(${start + index})'
              : name;
      ...
      final start = startItem ?? 0;
      return Future.wait(
        associations.mapIndexed((index, mapEntry) async {
      ...
      final variables = await Future.wait(
        fields.mapIndexed((index, field) async {
      ...
      final includeGetters =
          (_adapter.args.showGettersInDebugViews ?? false) ||
      ...
      if (service != null && includeGetters) {
        ...
        final getters = await _gettersForClassHierarchy(
          thread,
          instance.classRef,
        );
        ...
        variables.removeWhere(
          (variable) => getterNames.contains(variable.name),
        );
  ...
  Future<Variable> createVariableForGetter(
    ...
    final fieldEvaluateNameString = _adapter.combineEvaluateName(
      evaluateName,
      '.$name',
    );
  ...
  Future<dap.Variable> convertVmResponseToVariable(
    ...
    if (response is vm.InstanceRef) {
      ...
      return dap.Variable(
        ...
        indexedVariables: response.isList || response.isSet
            ? response.length
            : null,
        variablesReference: variablesReference,
      );
      ...
      final errorMessage = _adapter.extractUnhandledExceptionMessage(
  ...
  Future<dap.StackFrame> convertVmToDapStackFrame(
    ...
    final sourcePathUri = uri != null
        ? await thread.resolveUriToPath(uri)
        : null;
  ...
  Future<Set<GetterInfo>> _gettersForClassHierarchy(
    ...
    while (service != null && classRef != null) {
      ...
      if (functions != null) {
        final instanceFields = functions.where(
          (f) =>
              (f.isGetter ?? false) &&
              !(f.isStatic ?? false) &&
              !(f.isConst ?? false),
