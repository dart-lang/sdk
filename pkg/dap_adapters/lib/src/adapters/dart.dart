...
abstract class DartDebugAdapter<
  TL extends LaunchRequestArguments,
...
>
  ...
  /// will return null.
  EvaluateName? getEvaluateName({required String? instanceRefId}) {
  ...
  String? buildEvaluateName(
    ...
    final parentEvaluateName = getEvaluateName(
      instanceRefId: parentInstanceRefId,
    );
  ...
  Future<void> connectDebugger(Uri uri) async {
    try {
      ...
      final message =
          'Failed to connect/initialize debugger for $uri:\n'
          '$error\n$stack';
      logger?.call(message);
  ...
  void sendDebuggerUris(Uri uri) {
    ...
    // this app).
    sendEvent(
      RawEventBody({'vmServiceUri': uri.toString()}),
  ...
  Future<void> _configureExistingIsolates(
    ...
    final existingIsolates = existingIsolateRefs != null
        ...
        : <vm.Isolate>[];
    await Future.wait(
      existingIsolates.map((isolate) async {
  ...
  Future<void> evaluateRequest(
    ...
    final isExceptionExpression =
        expression == threadExceptionExpression ||
  ...
  Future<void> stackTraceRequest(
    ...
    if (thread == null) {
      if (isolateManager.isInvalidThreadId(threadId)) {
        ...
        // response.
        sendResponse(StackTraceResponseBody(stackFrames: [], totalFrames: 0));
        return;
      }
    }
    ...
    if (data is FrameScopeData && data.kind == FrameScopeDataKind.locals) {
        ...
        Future<Variable> convert(int index, vm.BoundVariable variable) {
          ...
          return _converter.convertVmResponseToVariable(
            ...
            evaluateName: name != null
                ? (evaluateName: name, isNullable: false)
                : null,
            format: format,
          );
        }
      ...
      if (key is vm.InstanceRef && value is vm.InstanceRef) {
        ...
        variables.addAll([
          ...
          Variable(
            ...
            evaluateName: buildEvaluateName('', parentInstanceRefId: value.id),
          ),
        ]);
      }
      ...
      try {
        ...
        if (object is vm.Sentinel) {
          ...
          variables.addAll(
            await _converter.convertVmInstanceToVariablesList(
              thread,
              object,
  ...
  Future<List<OutputEventBody>> _buildOutputEventsWithSourceReferences(
    ...
    // frames.
    final framePaths = await Future.wait(
      frameLocations.map((frame) async {
        final uri = frame?.uri;
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      events.add(
        OutputEventBody(
          category: category,
          output: output,
          source: clientPath != null
              ? Source(name: name, path: clientPath)
              : null,
  ...
  Future<vm.Response?> _evaluateExceptionExpression(
    ...
    final exception =
        isolateManager.getStoredData(exceptionReference)?.data
            as vm.InstanceRef?;
    ...
    // by its ID.
    final expressionWithoutExceptionExpression = expression.substring(
      threadExceptionExpression.length + 1,
    );
  ...
  Future<void> handleToolEvent(vm.Event event) async {
    ...
    sendEvent(
      RawEventBody({'kind': event.extensionKind, 'data': data}),
