...
abstract class BaseDebugAdapter<
...
> {
  ...
  void handleIncomingRequest(
    ...
    if (request.command == 'initialize') {
      ...
      handle(request, threadsRequest, _voidArgs, responseWriter);
      ...
      handle(request, sourceRequest, SourceArguments.fromJson, responseWriter);
    } else if (request.command == 'scopes') {
      handle(request, scopesRequest, ScopesArguments.fromJson, responseWriter);
  ...
  void _handleIncomingResponse(Response response) {
    final completer = _serverToClientRequestCompleters.remove(
      response.requestSeq,
    );
  ...
  _VoidArgRequestHandler<TArg> _withVoidResponse<TArg>(
    ...
    return (request, arg, sendResponse) =>
        handler(request, arg, () => sendResponse(null));
  }
}
