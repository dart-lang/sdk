...
///     { JSON payload }
///
/// into just the JSON payload, decoded with the specified encoding. Line
/// endings
...
class PacketTransformer extends StreamTransformerBase<List<int>, String> {
              ...
              (codeUnit) {
                ...
                try {
                  if (isParsingHeaders && _endsWithCrLfCrLf(buffer)) {
                    ...
                    // UTF-8 is the default - and only supported - encoding for
                    // LSP.
                    // The string 'utf8' is valid since it was published in the
                    // original spec.
  ...
  /// Decodes [buffer] into a String and returns the 'Content-Length' header
  /// value.
  static ProtocolHeaders _parseHeaders(List<int> buffer) {
    ...
    try {
      ...
      throw FormatException(
        'Unable to decode headers with ascii. '
