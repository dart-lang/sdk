...
abstract class CommonWebSocketVM extends VM {
  ...
  void _notifyConnect() {
    if (!_connected.isCompleted) {
      Logger.root.info(
        'WebSocketVM connection opened: ${target.networkAddress}',
      );
  ...
  void _notifyDisconnect(String reason) {
    if (!_disconnected.isCompleted) {
      Logger.root.info(
        'WebSocketVM connection error: ${target.networkAddress}',
      );
  ...
  void _onBinaryMessage(dynamic data) {
    _webSocket!.nonStringToByteData(data).then((ByteData bytes) {
      ...
      var metadata = _utf8Decoder.convert(
        new Uint8List.view(
          bytes.buffer,
          bytes.offsetInBytes + metadataOffset,
          metadataLength,
        ),
      );
  ...
  void _cancelRequests(
    Map<String, _WebSocketRequest> requests,
    String message,
  ) {
    requests.forEach((String serial, _WebSocketRequest request) {
      var exception = new NetworkRpcException(
        message +
