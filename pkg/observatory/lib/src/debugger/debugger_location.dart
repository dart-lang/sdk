...
class DebuggerLocation {
  ...
  static RegExp packageLocMatcher = new RegExp(
    r'^package:([^\d:][^:]+:)?(\d+)(:\d+)?',
  );
  ...
  static Future<Set<Script>> _lookupScript(
    Isolate isolate,
    String name, {
  ...
  static List<ServiceFunction> _lookupFunction(
    Isolate isolate,
    String name, {
    bool allowPrefix = false,
  }) {
  ...
  static Future<List<Class>> _lookupClass(
    Isolate isolate,
    String name, {
    bool allowPrefix = false,
  }) async {
  ...
  static RegExp partialSourceLocMatcher = new RegExp(
    r'^([^\d:]?[^:]+[:]?)?(\d+)?([:]\d*)?',
  );
  ...
  static Future<List<String>> _completeFile(
    ...
    if (!scriptNameComplete) {
      ...
      var scripts = await _lookupScript(
        debugger.isolate,
        scriptName,
        allowPrefix: true,
      );
      ...
      if (!lineStrComplete) {
        ...
        var completions = <String>[];
        var report =
            await script.isolate!.getSourceReport([
