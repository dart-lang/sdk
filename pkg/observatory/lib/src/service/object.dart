...
Future cancelFutureSubscription(
  Future<StreamSubscription> subscriptionFuture,
) async {
...
abstract class ServiceObject implements M.ObjectRef {
  ...
  Future<Map> _fetchDirect({int count = kDefaultFieldLimit}) {
    Map params = {'objectId': id, 'count': count};
...
abstract class VM extends ServiceObjectOwner implements M.VM {
  ...
  void _buildIsolateList() {
    var isolateList = _isolateCache.values
        .where((i) => !i.isSystemIsolate!)
        .toList();
    isolateList.sort(_compareIsolates);
    isolates.clear();
    isolates.addAll(isolateList);

    var systemIsolateList = _isolateCache.values
        .where((i) => i.isSystemIsolate!)
        .toList();
    ...
    if ((id != null)) {
      ...
      if (id.startsWith(_isolateGroupIdPrefix)) {
        ...
        if (isolateGroup == null) {
          ...
          // Eagerly load the isolate.
          isolateGroup.load().then(
            (_) {},
            onError: (e, stack) {
              Logger.root.info(
                'Eagerly loading an isolate group failed: $e\n$stack',
              );
            },
          );
        } else {
  ...
  void _removeDeadIsolateGroups(List newIsolateGroups) {
    // Build a set of new isolates.
    final Set newIsolateGroupSet = newIsolateGroups
        .map((iso) => iso.id)
        .toSet();
  ...
  Future<Map> invokeRpcNoUpgrade(String method, Map params) {
    return invokeRpcRaw(method, params)
        .then<Map>((Map response) {
          var map = response;
          var tracer = Tracer.current;
          if (tracer != null) {
            tracer.trace(
              "Received response for ${method}/${params}}",
              map: map,
            );
          }
          if (!_isServiceMap(map)) {
            var exception = new MalformedResponseRpcException(
              "Response is missing the 'type' field",
              map,
            );
  ...
  Future<StreamSubscription> listenEventStream(
    String streamId,
    void Function(ServiceEvent) function,
  ) async {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    _removeDeadIsolates([...map['isolates'], ...map['systemIsolates']]);
  ...
  Future reloadIsolates() {
    ...
    for (var isolate in isolates) {
      var reload = isolate.reload().then(
        (_) {},
        onError: (e) {
...
class Ports implements M.Ports {
  ...
  Ports(ServiceMap map)
    : this.elements = map['ports'].map<Port>((rmap) => new Port(rmap)).toList();
}
...
class Port implements M.Port {
  ...
  Port(ServiceMap map) : name = map['name'], handler = map['handler'];
}
...
class Isolate extends ServiceObjectOwner implements M.Isolate {
  ...
  Future<ServiceObject> getSourceReport(
    List<String> report_kinds, [
    Script? script,
    int? startPos,
    int? endPos,
  ]) {
  ...
  Future<ServiceObject> getObject(
    String objectId, {
    bool reload = true,
    ...
    Map params = {'objectId': objectId, 'count': count};
  ...
  Future<Breakpoint> addBreakpoint(Script script, int line, [int? col]) {
    Map params = {'scriptId': script.id, 'line': line};
    if (col != null) {
      params['column'] = col;
    }
    return invokeRpc(
  ...
  Future<Breakpoint> addBreakpointByScriptUri(
    String uri,
    int line, [
    int? col,
  ]) {
    Map params = {'scriptUri': uri, 'line': line.toString()};
    if (col != null) {
  ...
  Future<ServiceObject> invoke(
    ServiceObject target,
    String selector, [
  ...
  Future<ServiceObject> eval(
    ...
    try {
      ...
      if (error.code == ServerRpcException.kExpressionCompilationError) {
        final String details = error.data != null
  ...
  Future<ServiceObject> evalFrame(
    int frameIndex,
    String expression, {
    Map<String, ServiceObject>? scope,
    bool disableBreakpoints = false,
  ...
  Future<ServiceObject> getTypeArgumentsList(bool onlyWithInstantiations) {
    Map params = {'onlyWithInstantiations': onlyWithInstantiations};
  ...
  Future<Map<String, ServiceMetric>> _refreshMetrics(
    ...
    return invokeRpc('_getIsolateMetricList', {'type': metricType}).then((
      dynamic result,
    ) {
      // Clear metrics map.
      metricsMap.clear();
...
class ServiceEvent extends ServiceObject {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    if (map['logRecord'] != null) {
      ...
      logRecord!['time'] = new DateTime.fromMillisecondsSinceEpoch(
        logRecord!['time'],
      );
...
class Breakpoint extends ServiceObject implements M.Breakpoint {
  ...
  Future<void> setState(bool enable) {
    return location!.script!.isolate!
        .invokeRpcNoUpgrade('setBreakpointState', {
...
class Class extends HeapObject implements M.Class {
  ...
  void _update(Map map, bool mapIsRef) {
    ...
    traceAllocations = (map['traceAllocations'] != null)
        ? map['traceAllocations']
        : false;
  }
...
class Script extends HeapObject implements M.Script {
  ...
  List<LocalVarLocation> scanLineForLocalVariableLocations(
    Pattern pattern,
    String name,
    String lineContents,
    int lineNumber,
    ...
    pattern.allMatches(lineContents).forEach((Match match) {
      ...
      var localVarLocation = new LocalVarLocation(
        lineNumber,
        column,
        endColumn,
      );
  ...
  List<LocalVarLocation> scanForLocalVariableLocations(
    ...
    if (!getLine(line)!.isTrivial) {
      ...
      r.addAll(
        scanLineForLocalVariableLocations(
          pattern,
          name,
          lineContents,
          line++,
    ...
    if (!getLine(line)!.isTrivial) {
      ...
      r.addAll(
        scanLineForLocalVariableLocations(pattern, name, lineContents, line, 0),
      );
    }
    return r;
  }
...
class Code extends HeapObject implements M.Code {
  ...
  void _processInline(
    List /*<ServiceFunction>*/ inlinedFunctionsTable,
  ...
  void _processDisassembly(List disassembly) {
    instructions.clear();
    instructionsByAddressOffset = new List<CodeInstruction?>.filled(
    ...
    for (var i = 0; i < disassembly.length; i += 4) {
      ...
      var instruction = new CodeInstruction(
        address,
        pcOffset,
        machine,
        human,
...
class TimelineFlags implements M.TimelineFlags {
  ...
  static final Set<String> _dart = new Set<String>.from(const <String>[
    'GC',
    'Compiler',
    'Dart',
  ]);
  ...
  // Dart developers care about the following streams:
  static final Set<String> _flutter = new Set<String>.from(const <String>[
    'GC',
    'Dart',
    'Embedder',
  ]);
  ...
  factory TimelineFlags(ServiceMap response) {
    ...
    final TimelineRecorder recorder = new TimelineRecorder(
      response['recorderName'],
    );

    assert(response['recordedStreams'] != null);
    final Set<String> recorded = new Set<String>.from(
    ...
    final List<TimelineProfile> profiles = [
      ...
      new TimelineProfile(
        'VM Developer',
        streams.where((s) => _vm.contains(s.name)).toList(),
      ),
