...
class AnalyzeCommand extends DartdevCommand<int> {
  ...
  FutureOr<int> run() async {
    if (argResults.rest.length > 1) {
      log.stderr("Only one directory is expected.");
      printUsage();
      return 1;
    }

    // find directory from argResults.rest
    var dirSet = Set<Directory>.from(
        argResults.rest.map<Directory>((String path) => Directory(path)));

    var dir;
    if (dirSet.length == 1) {
      dir = dirSet.single;
      if (!dir.existsSync()) {
        log.stderr('${dir.path} is not a directory on this machine.');
        printUsage();
        return 1;
      }
    } else {
...
class AnalysisError implements Comparable<AnalysisError> {
  ...
  String toDisplayString() {
    // TODO(jwren) Re-visit this to add ANSI color back when applicable.
    final String padding = ' ' * math.max(0, 7 - severity.length);
    return '$padding${severity.toLowerCase()} $_separator '
        '$messageSentenceFragment $_separator '
        '${path.relative(file)}:$startLine:$startColumn $_separator '
        '$code';
  }
