...
''');
        ...
        final canonicalDeps = deps?.map(p.canonicalize).toList();
        expect(canonicalDeps, contains(p.canonicalize(scriptFile.path)));
        expect(canonicalDeps, contains(p.canonicalize(helperFile.path)));
...
''');
      ...
      expect(result.stdout.trim(), equals('Hello from bar:hello'));
    });
...
''');
        ...
        expect(result.stdout, startsWith('SCRIPT: '));
        expect(
          p.canonicalize(result.stdout.trim().substring('SCRIPT: '.length)),
          equals(expectedPath),
        );
...
''');
        ...
        expect(result.exitCode, 0);
        expect(result.stdout.trim(), equals('Hello 1'));
...
''');
        ...
        expect(result.exitCode, 0);
        expect(result.stdout.trim(), equals('Hello 2'));
      },
    );
...
''');
        ...
        expect(result.exitCode, 0);
        expect(result.stdout.trim(), equals('Hello experiment'));
