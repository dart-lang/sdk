...
void debugAdapter() {
  ...
  for (final commandName in ['debug-adapter', 'debug_adapter']) {
    test('$commandName --help', () async {
      final p = project();
      var result = await p.run([commandName, '--help']);
    ...
    test('$commandName invalid input provides a suitable message', () async {
      final p = project();
      final process = await p.start([commandName]);
